...
# Object considered grasped (lifted) when world-frame z exceeds this (m); matches :meth:`get_termination`.
DEFAULT_GRASP_SUCCESS_HEIGHT = 0.85


class LocoGraspEnv(GraspEnv):
    """
    Environment for loco (mobile) dexterous grasping with Dex3/G1 hand.

    Inherits from GraspEnv. Use get_grasp_pose(hand_type="dex3_1") to retrieve
    dex3_1_grasp_pose annotations (functional_grasp/grasp with coarse/fine/final phases).
    """
    ...
    # ------------------------------------------------------------------
    # Grasp success (for Collect / LocoDexGrasp)
    # ------------------------------------------------------------------
    ...
    def get_grasp_success(
        ...
        height_above_floor: float = 0.85,
    ) -> torch.Tensor:
        """
        Per-env success: target object is lifted at least ``height_above_floor`` (m) in world z.

        Default matches :meth:`get_termination` (table scene: lift above ~0.85 m). Floor reference
        is z == 0, same as :class:`SquatGraspEnv` but with a higher threshold for loco/table tasks.
        """
        if env_ids is None:
    ...
    def get_eef_pose(self, env_ids: Sequence[int] | None = None) -> torch.Tensor:
        ...
        # Single-arm: [N, 3] + [N, 4] -> [N, 7]; dual-arm: [N, 2, 3] + [N, 2, 4] -> [N, 2, 7]
        if eef_pos.dim() == 2:
    ...
    def get_termination(self):
        ...
        object_z = object_pos[:, 2]
        termination = (distance < 0.6) & (object_z > 0.85)
        truncated = object_z < 0.2
