"""
SquatGraspEnv：仅仿真 + 监测瓶子位姿。

首次记录 bottle_ref 后会立刻做一次 IK goalset + 可视化（否则瓶子不动时 d_ref≈0，不会触发「移动且稳定」条件）。
...
def _resolve_robot_name(env: SquatGraspEnv) -> str:
...
def _get_robot_state_dict(env: SquatGraspEnv) -> dict:
...
def _get_ik_server_right_hand(env: SquatGraspEnv):
...
def _ik_select_best_coarse(
    env: SquatGraspEnv, coarse_poses: torch.Tensor, env_id: int = 0
) -> Tuple[bool, int]:
...
def _get_bottle_pose_7d(env: SquatGraspEnv) -> Optional[torch.Tensor]:
...
def _bottle_moved_from_reference(
    bottle_now: torch.Tensor, bottle_ref: torch.Tensor, pos_eps: float = 1e-3
) -> bool:
    if bottle_now is None or bottle_ref is None:
        return False
    return bool(torch.norm(bottle_now[:3] - bottle_ref[:3]) > pos_eps)


def _bottle_stable_vs_prev(
    bottle_now: torch.Tensor, bottle_prev: Optional[torch.Tensor], pos_eps: float = 1e-4
) -> bool:
    if bottle_now is None or bottle_prev is None:
        return False
    return bool(torch.norm(bottle_now[:3] - bottle_prev[:3]) < pos_eps)


def should_regenerate_grasp(
    env: SquatGraspEnv,
    bottle_ref: Optional[torch.Tensor],
    bottle_prev: Optional[torch.Tensor],
) -> bool:
    bottle_now = _get_bottle_pose_7d(env)
    if bottle_now is None or bottle_ref is None:
        return False
    if not _bottle_moved_from_reference(bottle_now, bottle_ref):
        return False
    if not _bottle_stable_vs_prev(bottle_now, bottle_prev):
        return False
    return True


def apply_goalset_and_visualize(env: SquatGraspEnv, device: torch.device) -> bool:
...
def main(cfg: DictConfig):
    ...
    bottle_ref: Optional[torch.Tensor] = None
    bottle_prev: Optional[torch.Tensor] = None
    for i in range(150):
    ...
    while True:
        env.step(action=neutral_action)
        bottle_now = _get_bottle_pose_7d(env)
        if bottle_now is None:
            continue

        if bottle_ref is None:
            bottle_ref = bottle_now.clone()
            bottle_prev = bottle_now.clone()
            apply_goalset_and_visualize(env, device)
            continue

        if should_regenerate_grasp(env, bottle_ref, bottle_prev):
