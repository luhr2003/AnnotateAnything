...
class DualLocoRetractReach(Task):
    """
    DualLocoRetractReach task for dual-arm mobile manipulation with retract behavior.
    Sends RetractReach commands with hand_id=-1 (dual-arm mode),
    ...
    def __init__(
        ...
        self.left_obj_id = int(getattr(config, "left_obj_id", 1))
        self.init_count = 0
    ...
    def step(self):
        if self.init_count < self.init_limit:
            ...
            # Dual-arm format: 9 elements with hand_id=-1, targets all None during init
            self.current_action = [
                "RetractReach",
                0,  # robot_id
                -1,  # hand_id=-1 (dual-arm)
                None,
                None,
                None,  # right target (None during init)
                None,
                None,
                None,  # left target (None during init)
            ]
            self.last_action = None
            return self.current_action
        else:
            self.current_state = "running"
            # Dual-arm format: 9 elements with both right and left targets
            self.current_action = [
                "RetractReach",
                0,  # robot_id
                -1,  # hand_id=-1 (dual-arm)
