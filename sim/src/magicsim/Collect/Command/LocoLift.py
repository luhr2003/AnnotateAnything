"""
LocoLift: bimanual box-squeeze task wrapping :class:`Lift`.

Same init-then-real pattern as :class:`LocoDexGrasp`: fires ``init_limit``
placeholder commands (with ``obj_name=None``) so the atomic skill emits
all-NaN :class:`MobileMoveL` targets while the env settles, then fires
the real ``Lift`` command with the resolved target object.
"""
...
class LocoLift(Task):
    """Mobile bimanual box squeeze (pre-grasp → squeeze → lift)."""
