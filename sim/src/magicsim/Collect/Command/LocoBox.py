"""
LocoBox: bimanual ground-box squeeze task wrapping :class:`LocoBox` atomic
skill (RetractMoveL → MoveL → MoveL).

Same init-then-real pattern as :class:`LocoLift`: fires ``init_limit``
placeholder commands (with ``obj_name=None``) so the atomic skill emits
all-NaN RetractMoveL targets while the env settles, then fires the real
``LocoBox`` command with the resolved target object.
"""
...
class LocoBox(Task):
    """Mobile bimanual ground-box squeeze (pre-grasp → squeeze → lift)."""
    ...
    def step(self):
        ...
        if self.init_count < self.init_limit:
            ...
            self.current_action = ["LocoBox", self.robot_id, None, None, None]
            self.last_action = None
