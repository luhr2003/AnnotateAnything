...
class FrameTransformer(SensorBase):
    ...
    The order of target frames in the output follows the order in the config's target_frames list (not alphabetical).
    """
    ...
    def find_bodies(
        ...
        return string_utils.resolve_matching_names(
            name_keys, self._target_frame_names, preserve_order
        )
    ...
    def _initialize_impl(self):
        ...
        # Order of (frame_name, body_name) following cfg.target_frames list order (not alphabetical)
        target_frame_order: list[tuple[str, str]] = []
        ...
        for frame, prim_path, offset, frame_type in zip(
            ...
            for prim in matching_prims:
                ...
                frame_name = (
                    ...
                    else matching_prim_path.rsplit("/", 1)[-1]
                )

                # Record target frame order following cfg.target_frames list order
                if frame_type == "target":
                    target_frame_order.append((frame_name, body_name))
        ...
        if "env_" in all_prim_paths[0]:
            ...
            self._per_env_indices = [
                index
                for index, _ in sorted(enumerate(all_prim_paths), key=lambda x: x[1])
            ]
            sorted_prim_paths = [
        ...
        # Order follows cfg.target_frames list (not alphabetical)
        ...
        # Build mapping from body_name to index in transform array (for target bodies only)
        body_name_to_index = {
            name: i for i, name in enumerate(self._target_frame_body_names)
        }

        # Iterate in cfg.target_frames order (not alphabetical) to preserve user-specified frame order
        for frame_name, body_name in target_frame_order:
            if frame_name in target_offsets:
                i = body_name_to_index[body_name]
                target_frame_offset_pos.append(target_offsets[frame_name]["pos"])
                target_frame_offset_quat.append(target_offsets[frame_name]["quat"])
        ...
        duplicate_frame_indices = torch.tensor(
            duplicate_frame_indices, device=self.device
        )
        if self._source_is_also_target_frame:
    ...
    def _debug_vis_callback(self, event):
        ...
        frames_pos = torch.cat(
            [self._data.source_pos_w, self._data.target_pos_w.view(-1, 3)], dim=0
        )
        frames_quat = torch.cat(
            [self._data.source_quat_w, self._data.target_quat_w.view(-1, 4)], dim=0
        )
    ...
    def _get_connecting_lines(
        ...
        default_direction = torch.tensor([0.0, 0.0, 1.0], device=self.device).expand(
            start_pos.size(0), -1
        )
