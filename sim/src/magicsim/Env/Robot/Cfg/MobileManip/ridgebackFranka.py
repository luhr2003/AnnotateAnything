...
class RidgebackFrankaPControllerHelper:
    """
    RidgebackFranka P-controller preprocess helper (stateful).

    Mirrors :class:`G1PControllerHelper`'s structure:

    * ``preprocess`` maps the 8-dim base action ``[x, y, z, qw, qx, qy, qz, lock_flag]``
      onto the 4-dim P-controller command ``[target_x, target_y, target_heading, mode_flag]``
      as a pure 1:1 pass-through of ``lock_flag`` ∈ ``{-1, 0}`` produced by
      ``MobileMoveL`` (no distance/heading auto-logic).
    * All-NaN rows (IK wait) force ``lock_flag = -1`` (lock_skip), and the target
      pose falls back through ``_last_target_*`` → current base pose, so the
      P-controller never chases a zeroed ``last_command``.
    * ``_last_*`` buffers are updated at the end of every ``preprocess`` call and
      cleared by ``reset_idx`` on env reset.
    """
    ...
    def preprocess(
        ...
        return ridgebackfranka_preprocess_p_controller_action(
            action,
            robot_state,
            env_ids,
            device=device,
    ...
    def move_strategy(
        ...
        **kwargs,
        ...
        """Stateless wrapper around :func:`ridgebackfranka_move_strategy`.

        Accepts and ignores G1-style kwargs (e.g. ``hand_id``) so the shared
        caller in ``RetractMoveL`` can invoke both helpers with a uniform
        signature.
        """
        return ridgebackfranka_move_strategy(trajectory, robot_state)
...
def ridgebackfranka_preprocess_p_controller_action(
    action: torch.Tensor,
    robot_state: Dict,
    env_ids: torch.Tensor,
    device: torch.device = torch.device("cuda:0"),
    ...
    """Preprocess 8-dim RidgebackFranka base action to 4-dim P-controller format.

    One-to-one pass-through of the ``lock_flag`` produced by ``MobileMoveL``
    (``_LOCK_FLAG_LOCK_SKIP=-1``, ``_LOCK_FLAG_NAV=0``). No ``distance`` /
    ``heading`` auto-logic: the planner is responsible for deciding when to
    navigate vs. hold.

    Stateful NaN fallbacks mirror ``G1PControllerHelper.preprocess``: if
    ``last_target_xy`` / ``last_target_yaw`` buffers are supplied, all-NaN rows
    (IK wait) fall back first to the cached last target and only then to the
    current base pose. The buffers are updated in place at the end of the call.

    Input:
        action: [N, 8] with ``[x, y, z, qw, qx, qy, qz, lock_flag]``.
            * ``lock_flag = -1``: lock_skip (hold, base is locked).
            * ``lock_flag = 0`` : nav (free base).
            * All-NaN row: treated as ``lock_flag = -1`` and target pose falls
              back to ``_last_*`` → current base pose.

    Returns:
        [N, 4] tensor ``[target_x, target_y, target_heading, mode_flag]`` where
        ``mode_flag`` is either ``-1`` (lock_skip) or ``0`` (nav).
    """
    ...
    # All-NaN row → forced lock_skip with cached/current pose fallback.
    ...
    # Remove Ry(-90°) offset between panda_mobile and base_link in the URDF.
    quat = action[:, 3:7].clone()  # [N, 4] (w, x, y, z)
    # Replace NaN quaternions with identity to avoid NaN propagation in quat_mul.
    # Rows with NaN quat (but non-NaN xy) get their target_yaw overridden to
    # current_yaw below — they signal "translate base, preserve heading"
    # (used by MobileServoL's linear-base synth, where the synthesized base
    # is already in base_link frame and shouldn't be re-rotated by the
    # MotionGen→base_link offset below).
    ...
    # NaN-quat rows: target_yaw → current_yaw ("preserve heading, just translate").
    if torch.any(nan_quat_mask):
        target_yaw = torch.where(nan_quat_mask, current_yaw, target_yaw)

    # All-NaN rows: fall back to cached last target, then current base pose.
    ...
    # Effective lock_flag: NaN rows forced to -1 (lock_skip) so we don't
    # navigate toward an undefined target. Matches G1PControllerHelper.preprocess.
    lock_flag = action[:, 7]
    lock_flag = torch.where(nan_mask, torch.tensor(-1.0, device=device), lock_flag)

    # Map lock_flag onto mode_flag:
    #   lock_flag < -0.5  → -1 (lock_skip)
    #   otherwise (nav)   → 0, but upgrade to 1 (turning) once base xy is
    #   already within `position_threshold` of target so we rotate in place
    ...
    # RidgebackFranka has no separate torso; heading = target yaw for all modes.
    target_heading = target_yaw

    result = torch.stack([target_x, target_y, target_heading, mode_flag], dim=1)

    # Write-through the buffers (store effective targets, same convention as G1).
    if last_target_xy is not None:
...
def ridgebackfranka_move_strategy(
    trajectory: torch.Tensor,
    robot_state: Dict[str, torch.Tensor],
) -> torch.Tensor:
    """
    Convert a MotionGen trajectory to a movement trajectory for RidgebackFranka.

    Unlike the G1 which needs stand-up/squat phases, the RidgebackFranka simply
    moves directly to the target position while keeping the EEF in a retracted
    (pending) position in front of the base.

    The retracted EEF pose is at base_pos + [0.2, 0, 0.35] with gripper pointing
    down (quat w,x,y,z = 0,1,0,0).
    ...
    # EEF retracted pose offset relative to base position
    # Position: [0.6, 0, 0.8] (0.6m forward, 0.8m above base)
    # Quaternion: gripper pointing down (w=0, x=1, y=0, z=0 → 180° around x)
    ...
    def _make_waypoint(lock_flag: float) -> torch.Tensor:
        """Create a [D+1] waypoint filled with NaN, lock_flag in last dim."""
        wp = torch.full((D + 1,), float("nan"), device=device, dtype=dtype)
        wp[-1] = lock_flag
        return wp

    def _fill_eef_pose(wp: torch.Tensor) -> None:
        """Fill EEF pose with retracted arm offset relative to base position."""
        if D >= 14:  # base (7) + arm (7)
            wp[7:10] = wp[0:3] + eef_offset[:3]  # pos = base_pos + offset
            wp[10:14] = eef_offset[3:]  # quat = downward
    ...
    for i in range(num_move_points):
        ...
        wp[:7] = trajectory[i, :7]  # base pose from trajectory
        _fill_eef_pose(wp)
        segments.append(wp)
...
RIDGEBACK_FRANKA_PANDA_CFG = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        usd_path=f"{MAGICSIM_ASSETS}/Robots/ridgebackfranka.usd",
...
RIDGEBACK_FRANKA_PINK_IK_CONTROLLER_CFG = PinkIKControllerCfg(
...
@configclass
class RidgebackFrankaActionsCfg(MobileManipActionsCfg):
    available_action: Dict[str, Dict[str, ActionTerm]] = {
        ...
        "arm_action": {
            ...
            "ik_abs": mdp.DifferentialInverseKinematicsActionCfg(
                ...
                command_reference_body_name="panda_link0",  # 机械臂底座，解 IK 相对此 link
            ...
            "ik_pink": PinkInverseKinematicsActionCfg(
                ...
                controller=RIDGEBACK_FRANKA_PINK_IK_CONTROLLER_CFG,
...
class RidgebackFrankaFrameCfg(FrameSensorCfg):
    def __post_init__(self):
        self.target_frames = [
        ...
        self.target_frames[1].prim_path = self.robot_prim_path + "/panda_link0"
...
# ================================
@configclass
class RidgebackFrankaPlannerCfg(MobileManipPlannerCfg):
    ...
    # P-controller helper: class with preprocess, reset_idx (no-op)
    p_controller_helper = RidgebackFrankaPControllerHelper
    p_controller_n_extra_dims: int = 0
    move_strategy = RidgebackFrankaPControllerHelper.move_strategy
    # Distance threshold for base movement to trigger move strategy
    move_strategy_distance_threshold: float = 0.1


@configclass
class RidgebackFrankaObsCfg(ObsGroup):
...
@configclass
class RidgebackFrankaCfg(RobotCfg):
    ...
    # Subconfigs
    action: RidgebackFrankaActionsCfg = MISSING
    ee_frame: RidgebackFrankaFrameCfg = MISSING
    obs: RobotObsCfg = MISSING
    planner: RidgebackFrankaPlannerCfg = RidgebackFrankaPlannerCfg()

    def __post_init__(self):
        self.robot: ArticulationCfg = RIDGEBACK_FRANKA_PANDA_CFG
        print("spawn: ", self.robot.spawn)

        # set prim paths
        self.robot.prim_path = self.prim_path
        # define actions and frames
        self.action = RidgebackFrankaActionsCfg(
        ...
        self.ee_frame = RidgebackFrankaFrameCfg(robot_prim_path=self.robot.prim_path)

        # observation configuration
        self.obs = RidgebackFrankaObsCfg(
