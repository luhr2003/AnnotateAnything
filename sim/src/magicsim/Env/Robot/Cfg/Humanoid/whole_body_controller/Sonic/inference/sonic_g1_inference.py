...
it. It consumes the retargeted robot trajectory directly. Architecturally it
only differs from VR3PT / SMPL paths in `_build_encoder_obs` — the proprio
layout, history buffers, decoder call, and action scaling are all identical
(decoder_dyn.onnx is shared).
...
semantically weird but deterministic:
    ...
    motion_anchor_ori_b_mf_nonflat (observations.py:1022-1043, commands.py:1942-1963)
        ...
        ref_root_quat is the ROBOT RETARGET root (hybrid path: planner cache's
        world-frame root quat, frames[..., 3:7]). No Y→Z, no base-rot removal.
...
**原样迁自** `GR00T-WholeBodyControl/sonic_python_inference/sonic_g1_inference.py`。
唯一改动：`from .sonic_inference import ...` → `from .quat_utils import ...`。
"""
...
G1_ENCODER_INPUT_DIM = G1_NUM_FUTURE_FRAMES * (
...
def _root_ori_b_6d_per_frame(
    ...
    ref_root_quat_future_wxyz: torch.Tensor,  # [N, F, 4] retarget root quat
) -> torch.Tensor:
...
class SonicG1Inference:
    ...
    Usage:
        infer = SonicG1Inference(
            num_envs=4,
            g1_encoder_onnx="Assets/WBC/models/sonic_v1/g1_encoder_dyn.onnx",
            decoder_onnx="Assets/WBC/models/sonic_v1/decoder_dyn.onnx",
        ...
        for policy_tick in range(T):
            targets = infer.step(
                ...
                ref_root_quat_future_wxyz=...,   # [N, 10, 4]  robot pkl / planner root quat (wxyz)
                joint_pos=..., joint_vel=...,
    ...
    def _build_encoder_obs(
        ...
        # Reproduce observations.py:584-587 non_flatten=True reshape.
        cmd_nonflat = cmd_flat.reshape(self.N, G1_NUM_FUTURE_FRAMES, -1)  # [N, F, 58]
    ...
    def step(
        ...
        return (
            self.default_angles.unsqueeze(0) + self.action_scale.unsqueeze(0) * action
        )
