...
class DifferentialIKController:
    ...
    def compute(
        ...
        ee_pos_des: torch.Tensor | None = None,
        ee_quat_des: torch.Tensor | None = None,
    ) -> torch.Tensor:
        ...
        Args:
            ...
            ee_pos_des: Optional desired end-effector position in shape (N, 3). When provided,
                must match the batch size of ee_pos. If None, uses the pose set by set_command().
            ee_quat_des: Optional desired end-effector orientation in shape (N, 4). When provided,
                must match the batch size of ee_quat. If None, uses the pose set by set_command().

        Returns:
            The target joint positions commands in shape (N, num_joints).
        """
        pos_des = ee_pos_des if ee_pos_des is not None else self.ee_pos_des
        quat_des = ee_quat_des if ee_quat_des is not None else self.ee_quat_des
        # compute the delta in joint-space
        if "position" in self.cfg.command_type:
            position_error = pos_des - ee_pos
