...
class PinkIKController:
    ...
    def __init__(
        ...
        urdf_path = self._resolve_path(cfg.urdf_path)
        mesh_path = self._resolve_path(cfg.mesh_path)
    ...
    def _resolve_path(path: str | None) -> str | None:
        if path is None:
            return None
        resolved = (
            path.replace("$MAGICSIM_ASSETS", MAGICSIM_ASSETS)
            .replace("{MAGICSIM_ASSETS}", MAGICSIM_ASSETS)
            .replace("{MAGIC_ASSET}", MAGICSIM_ASSETS)
        )
        if not os.path.isabs(resolved):
            resolved = os.path.join(MAGICSIM_HOME, resolved.lstrip("./"))
        return os.path.normpath(resolved)

    def _validate_consistency(
        self, cfg: PinkIKControllerCfg, controlled_joint_indices: list[int]
    ) -> None:
    ...
    def _setup_joint_ordering_mappings(self):
        ...
        Note: Isaac Lab may have joints (e.g. mobile base dummy joints) that do not exist
        in the Pink URDF. The mappings only cover the intersection of joints present in
        both Isaac Lab and the Pink URDF.
        """
        ...
        # Create reordering array: for each Pink URDF joint, find its index in the
        # Isaac Lab joint list.  This is used to extract the relevant joint positions
        # from the full Isaac Lab joint array and place them in Pink/Pinocchio order.
        self.isaac_lab_to_pink_ordering = np.array(
            [isaac_lab_joint_names.index(pink_joint) for pink_joint in pink_joint_names]
        )
        # Reverse mapping: for each Isaac Lab joint that exists in the Pink URDF,
        # find its index in the Pink joint list.  Joints present only in Isaac Lab
        # (e.g. mobile-base dummy joints) are skipped.
        self.pink_to_isaac_lab_ordering = np.array(
            [
                ...
                if isaac_lab_joint in pink_joint_names
            ]
        )
        # Create reordering arrays for controlled joints only
        pink_controlled_joint_names = (
            self.pink_configuration.controlled_joint_names_pinocchio_order
        )
    ...
    def compute(
        ...
        try:
            ...
            joint_angle_changes = velocity * dt * self.cfg.amplify_factor
