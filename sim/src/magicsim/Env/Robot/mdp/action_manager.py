...
from isaaclab.managers.action_manager import ActionTerm as IsaacActionTerm
...
class ActionTerm(ManagerTermBase):
    ...
    * Applying actions: This operation is performed once per **simulation step** and is
    ...
    def _replace_nan_actions(
        self,
        actions: torch.Tensor,
        action_dim: int | None = None,
    ) -> torch.Tensor:
        """Replace NaN elements in actions with previous raw_actions. No env filtering.

        Used by WBC-style action terms that process all envs and need NaN fill without
        filtering. Returns the modified actions tensor.

        Args:
            actions: Input action tensor of shape (num_envs, action_dim).
            action_dim: If set, only process actions[:, :action_dim].

        Returns:
            Modified actions tensor with NaN replaced by previous raw_actions.
        """
        result = actions.clone()
        prev = self.raw_actions
        if action_dim is not None:
            work = result[:, :action_dim]
            prev = prev[:, :action_dim]
        else:
            work = result
    ...
    def _handle_nan_actions(
        ...
        """Handle NaN actions mimicking pink_task_space_actions._handle_nan_actions.

        Logic:
        1. Skip envs where ALL actions are NaN (return filtered env_ids).
        2. For remaining envs: element-wise fill NaN with previous raw actions.
        3. Optional group_size (e.g. 7 for pose): process in groups.

        Args:
            actions: Input action tensor of shape (num_envs, action_dim).
            selected_env_ids: Selected environment IDs.
            action_dim: If set, only process actions[:, :action_dim].
            group_size: If set (e.g. 7 for pose groups), process in groups.
        ...
        previous_actions = self.raw_actions[env_ids]

        if action_dim is not None:
            work = valid_actions[:, :action_dim].clone()
            prev = previous_actions[:, :action_dim]
        else:
            work = valid_actions.clone()
        ...
        if nan_mask.any():
            if (
                ...
                and group_size > 0
                and work.shape[1] >= group_size
            ):
                num_groups = work.shape[1] // group_size
                for g in range(num_groups):
                    start, end = g * group_size, (g + 1) * group_size
                    group_nan = torch.isnan(work[:, start:end]).any(dim=1)
                    if group_nan.any():
                        work[group_nan, start:end] = prev[group_nan, start:end]
                remainder_start = num_groups * group_size
                if remainder_start < work.shape[1]:
                    rem_nan = torch.isnan(work[:, remainder_start:]).any(dim=1)
                    if rem_nan.any():
                        work[rem_nan, remainder_start:] = prev[
                            rem_nan, remainder_start:
                        ]
            else:
                work[nan_mask] = prev[nan_mask]

        if action_dim is not None:
            valid_actions[:, :action_dim] = work
        else:
            valid_actions = work
...
class ActionManager(ManagerBase):
    ...
    * apply actions: This operation typically sets the processed actions into the assets in the
    ...
    def get_info(self):
        info = {}
        if "base_action" in self._terms:
            info["base_action"] = self._terms["base_action"].get_info()
        else:
            info["base_action"] = None
        if "arm_action" in self._terms:
            info["arm_action"] = self._terms["arm_action"].get_info()
        else:
            info["arm_action"] = None
        if "eef_action" in self._terms:
            info["eef_action"] = self._terms["eef_action"].get_info()
        else:
            info["eef_action"] = None
        return info
    ...
    def _prepare_terms(self):
        ...
        for term_name, term_cfg in cfg_items:
            ...
            if not isinstance(term, (ActionTerm, IsaacActionTerm)):
                raise TypeError(
    ...
    def get_raw_action(self, env_ids: Sequence[int] = None):
        ...
        for term_name, term in self._terms.items():
            ...
            # If env_ids is provided, index the tensor to only include those environments
    ...
    def get_processed_action(self, env_ids: Sequence[int] = None):
        ...
        for term_name, term in self._terms.items():
            ...
            if isinstance(processed_actions, torch.Tensor):
                assert processed_actions.shape[0] == self.num_envs, (
                    f"[{term_name}] _processed_actions shape[0] must be num_envs "
                    f"({self.num_envs}), got {processed_actions.shape[0]}. "
                    f"Use in-place updates (e.g. self._processed_actions[env_ids] = ...) "
                    f"instead of reassignment."
                )
                if env_ids is not None:
                    processed_actions = processed_actions[env_ids]
