...
INITIAL_JOINT_ANGLE = 30
INITIAL_JOINT_ANGLE_RATIO = 0.4
INITIAL_JOINT_POSITION_M = 0.4
INITIAL_JOINT_POSITION_M_RATIO = 0.25

#Target Points Sampling Constants
PANEL_NORMAL_ANGLE_DEG = 10
PANEL_INNER_KEEP_RATIO = 0.5
HINGE_KEEP_FAR_RATIO = 0.6
NUM_POINTS_PER_JOINT = 1000

#OVERSHOOT CONSTANTS
CLOSE_OVERSHOOT_DEG = 5.0   
CLOSE_OVERSHOOT_M   = 0.02 
...
SINGLE_OBJECT_USD = _path_from_env("CLOSE_PUSH_OBJECT_USD", _THIS_DIR / "45247" / "Object.usd")

INPUT_DATASET_PATH = _path_from_env("CLOSE_PUSH_DATASET_PATH", _THIS_DIR)
GRIPPER_USD = _path_from_env("CLOSE_PUSH_GRIPPER_USD", _WORKSPACE_ROOT / "Flying_hand_probe_pro.usd")
...
def save_trajectories_to_json(
    ...
    # Save to JSON
    json_path = annotation_dir / "close_by_push_trajectory.json"
...
def compute_door_normal_from_joint(
    ...
    # --- Debug draw: axis / radial / door_normal ---
    # Use bbox diagonal for scale (fallback 0.25)
...
def collect_mesh_data_under(root_path: str):
    """
    Collect all mesh vertices (world coords) and triangulated faces.

    Returns
    -------
    points : (N, 3) float64
    faces  : list[list[int]]  – each sub-list has 3 indices (triangulated)
    """
    stage = omni.usd.get_context().get_stage()
    root_prim = stage.GetPrimAtPath(root_path)
    if not root_prim.IsValid():
        raise ValueError(f"Root prim '{root_path}' is not valid.")
    ...
    for prim in Usd.PrimRange(root_prim):
        ...
        world_pts = [xf.Transform(p) for p in pts]
        ...
        for c in counts:
            ...
            v0 = offset + indices[cursor]
            for k in range(1, c - 1):
                v1 = offset + indices[cursor + k]
                v2 = offset + indices[cursor + k + 1]
    ...
    if not all_points:
        raise RuntimeError(f"No mesh points found under '{root_path}'")

    points = np.array([[p[0], p[1], p[2]] for p in all_points], dtype=np.float64)
    return points, faces

def weld_vertices(points: np.ndarray, faces: list, eps: float = 1e-4):
    """Merge vertices within eps distance. Rebuild faces; drop degenerate tris."""
    inv_eps = 1.0 / eps
    key_to_new = {}
    new_pts = []
    old_to_new = np.empty(len(points), dtype=np.intp)

    for i, p in enumerate(points):
        key = (
            int(round(p[0] * inv_eps)),
            int(round(p[1] * inv_eps)),
            int(round(p[2] * inv_eps)),
        )
    ...
    new_faces = []
    for f in faces:
        remapped = [int(old_to_new[v]) for v in f]
        if len(set(remapped)) == len(remapped):
            new_faces.append(remapped)

    return np.array(new_pts, dtype=np.float64), new_faces

def compute_face_geometry(points: np.ndarray, faces: list):
    """
    Returns
    -------
    centers : (F, 3)
    ...
    normals = np.empty((F, 3), dtype=np.float64)
    areas   = np.empty(F, dtype=np.float64)

    for i, face in enumerate(faces):
        v = points[face]
        centers[i] = v.mean(axis=0)
        e1 = v[1] - v[0]
        e2 = v[2] - v[0]
        cross = np.cross(e1, e2)
        mag = np.linalg.norm(cross)
        areas[i] = 0.5 * mag
        normals[i] = cross / mag if mag > 1e-12 else 0.0

    return centers, normals, areas

def select_panel_faces(
    centers: np.ndarray,
    normals: np.ndarray,
    areas: np.ndarray,
    faces: list,
    outward_normal: np.ndarray,
    normal_angle_deg: float = 10.0,
    n_bins: int = 64,
    band_factor: float = 1.5,
) -> np.ndarray:
    """
    Robustly select main panel faces, rejecting handles/knobs.

    1. Filter by normal alignment with outward_normal.
    2. Area-weighted depth histogram → peak bin = panel.
    3. Expand peak while neighbour area >= 10% of peak.
    4. Return face indices in the depth band.
    """
    out_n = np.asarray(outward_normal, dtype=np.float64)
    out_n /= np.linalg.norm(out_n) + 1e-12

    # Normal alignment
    cos_thresh = np.cos(np.deg2rad(normal_angle_deg))
    dots = (normals * out_n[None, :]).sum(axis=1)
    front_mask = dots >= cos_thresh
    idx_front = np.where(front_mask)[0]

    if len(idx_front) == 0:
        front_mask = np.abs(dots) >= cos_thresh
        idx_front = np.where(front_mask)[0]
        if len(idx_front) == 0:
            raise RuntimeError("No front-facing faces found — check outward_normal.")

    front_centers = centers[idx_front]
    front_areas   = areas[idx_front]

    # Depth along outward normal
    ref_point = np.average(front_centers, axis=0, weights=front_areas + 1e-12)
    depths = (front_centers - ref_point[None, :]) @ out_n

    # Area-weighted histogram
    d_min, d_max = depths.min(), depths.max()
    actual_bins = max(16, min(n_bins, int(len(idx_front) / 5)))
    bin_edges = np.linspace(d_min - 1e-6, d_max + 1e-6, actual_bins + 1)
    bin_width = bin_edges[1] - bin_edges[0]

    bin_ids = np.clip(np.digitize(depths, bin_edges) - 1, 0, actual_bins - 1)
    area_per_bin = np.zeros(actual_bins, dtype=np.float64)
    for bi, a in zip(bin_ids, front_areas):
        area_per_bin[bi] += a

    # Peak + expand
    peak_bin = int(np.argmax(area_per_bin))

    # --- grow the peak interval until it covers a target fraction of total front-facing area ---
    target_area_frac = 0.85  # tune: 0.75 tighter (less thickness), 0.90 thicker (more robust)
    total_area = float(area_per_bin.sum()) + 1e-12

    lo = hi = peak_bin
    acc = float(area_per_bin[peak_bin])

    while acc < target_area_frac * total_area:
        left = float(area_per_bin[lo - 1]) if lo - 1 >= 0 else -1.0
        right = float(area_per_bin[hi + 1]) if hi + 1 < actual_bins else -1.0

        if left < 0.0 and right < 0.0:
            break

        # Expand toward the heavier neighbor bin
        if right >= left:
            hi += 1
            acc += float(area_per_bin[hi])
        else:
            lo -= 1
            acc += float(area_per_bin[lo])

    # Convert selected bins -> depth slab (optionally padded a bit)
    pad_bins = 1  # set 0..2; extra robustness for tilt
    lo2 = max(0, lo - pad_bins)
    hi2 = min(actual_bins - 1, hi + pad_bins)

    band_lo = bin_edges[lo2]
    band_hi = bin_edges[hi2 + 1]

    in_band = (depths >= band_lo) & (depths <= band_hi)
    panel_indices = idx_front[np.where(in_band)[0]]

    print(f"[DoorPush] Depth histogram: {actual_bins} bins, "
          f"peak bin {peak_bin}, "
          f"band [{lo}..{hi}], depth [{band_lo:.4f}, {band_hi:.4f}]")
    print(f"[DoorPush] Panel faces: {len(panel_indices)}/{len(idx_front)} front-facing")

    return panel_indices

def build_face_adjacency(faces: list, face_indices: np.ndarray) -> Dict[int, List[int]]:
    ...
    adj: Dict[int, List[int]] = {fi: [] for fi in face_indices}
...
def largest_face_component(
    ...
    """
    Return the face indices of the largest *area* connected component.

    panel_fi : (M,) indices of candidate panel faces
    face_adj : dict {face_idx: [neighbor face_idx, ...]}
    areas    : (F,) per-face areas for the full mesh
    """
    panel_fi = np.asarray(panel_fi, dtype=np.int64)
    visited = set()
    best_comp: list[int] = []
    best_area = 0.0
    ...
    for fi in panel_fi:
        ...
        while stack:
            f = stack.pop()
            comp.append(f)
            comp_area += float(areas[f])

            for nb in face_adj.get(f, []):
    ...
    return np.array(best_comp, dtype=np.int64)

def sample_panel_surface_points(
    points: np.ndarray,
    faces,
    panel_fi,
    n_points: int,
    ...
    """
    Sample points on the surface of faces[panel_fi] in an area-weighted way.
    Robust to faces/panel_fi being lists.
    """
    # --- Robustify inputs ---
    ...
    if panel_fi.size == 0:
        print("[DoorPush] WARNING: sample_panel_surface_points called with empty panel_fi")
        return np.empty((0, 3), dtype=np.float64)

    tris = faces[panel_fi]  # (M, 3)

    v0 = points[tris[:, 0]]  # (M, 3)
    v1 = points[tris[:, 1]]
    v2 = points[tris[:, 2]]
    ...
    if total_area < 1e-12:
        print("[DoorPush] WARNING: total panel area ~ 0 in sampling.")
        return np.empty((0, 3), dtype=np.float64)

    probs = tri_areas / (total_area + 1e-12)

    # Choose triangles by area
    ...
    samples = u * v0_s + v * v1_s + w * v2_s  # (n_points, 3)
    return samples

def _convex_hull_2d(pts_2d: np.ndarray) -> np.ndarray:
    """
    ...
    Returns:
        hull_indices: 1-D array of vertex indices in CCW order (no repeated last point)
    """
    n = pts_2d.shape[0]
    if n < 3:
        return np.arange(n)

    order = np.lexsort((pts_2d[:, 1], pts_2d[:, 0]))

    def cross(o, a, b):
        return (pts_2d[a, 0] - pts_2d[o, 0]) * (pts_2d[b, 1] - pts_2d[o, 1]) - \
               (pts_2d[a, 1] - pts_2d[o, 1]) * (pts_2d[b, 0] - pts_2d[o, 0])
...
def compute_boundary_edge_segments(
    points: np.ndarray,
    faces: list,
    panel_face_indices: np.ndarray,
) -> np.ndarray:
    """
    Extract the boundary edges of the panel mesh patch.

    A boundary edge is one shared by exactly 1 panel face.

    Returns
    -------
    edges : (E, 2, 3) array — each row is a pair of 3D endpoints
    """
    edge_count: Dict[Tuple[int, int], int] = {}
    for fi in panel_face_indices:
        f = faces[fi]
        n = len(f)
        for k in range(n):
            a, b = f[k], f[(k + 1) % n]
    ...
    for (a, b), cnt in edge_count.items():
        if cnt == 1:
            boundary_edges.append([points[a], points[b]])

    if not boundary_edges:
        return np.empty((0, 2, 3), dtype=np.float64)
    return np.array(boundary_edges, dtype=np.float64)


def filter_samples_to_inner_region(
    samples: np.ndarray,
    outward_normal: np.ndarray,
    panel_face_indices: np.ndarray,
    points: np.ndarray,
    ...
    """
    Keep the inner `keep_ratio` of surface samples by 2D boundary inset.

    Steps:
      1. Build in-plane basis (u, v) perpendicular to outward_normal.
      2. Extract the panel mesh boundary edges and project them to 2D.
      3. For each sample, compute its min distance to ANY boundary edge in 2D.
      4. Keep samples whose boundary distance >= (1 - keep_ratio) * max_distance.

    This naturally handles non-rectangular panels, holes, etc.

    Args:
        samples: (S, 3) dense surface samples
        outward_normal: (3,) unit panel normal
        panel_face_indices: face indices of the panel
        points: (N, 3) welded mesh vertices
        faces: triangle list
        keep_ratio: fraction of interior to keep (0.7 = discard outer 30%)

    Returns:
        inner_samples: (K, 3) filtered samples
    """
    ...
    # --- 1. In-plane basis ---
    tmp = np.array([1, 0, 0], dtype=np.float64) if abs(out_n[0]) < 0.9 else np.array([0, 1, 0], dtype=np.float64)
    u_axis = np.cross(out_n, tmp);  u_axis /= np.linalg.norm(u_axis) + 1e-12
    v_axis = np.cross(out_n, u_axis); v_axis /= np.linalg.norm(v_axis) + 1e-12

    # --- 2. Project samples to 2D ---
    samples_u = samples @ u_axis   # (S,)
    samples_v = samples @ v_axis   # (S,)

    # --- 3. Extract boundary edges and project to 2D ---
    boundary_edges_3d = compute_boundary_edge_segments(points, faces, panel_face_indices)

    if boundary_edges_3d.shape[0] == 0:
        # No boundary found (closed surface?) — fall back to convex hull of samples
        print("[DoorPush] No mesh boundary edges; falling back to convex hull inset.")
        pts_2d = np.column_stack([samples_u, samples_v])
        hull_idx = _convex_hull_2d(pts_2d)
        hull_pts = pts_2d[hull_idx]
        n_hull = len(hull_idx)
        # Build edge list from hull
        edge_u_a = hull_pts[:, 0]
        edge_v_a = hull_pts[:, 1]
        edge_u_b = np.roll(hull_pts[:, 0], -1)
        edge_v_b = np.roll(hull_pts[:, 1], -1)
    else:
        # Project 3D boundary edge endpoints to 2D
        # boundary_edges_3d is (E, 2, 3)
        edge_a = boundary_edges_3d[:, 0, :]   # (E, 3)
        edge_b = boundary_edges_3d[:, 1, :]   # (E, 3)

        edge_u_a = edge_a @ u_axis   # (E,)
        edge_v_a = edge_a @ v_axis
        edge_u_b = edge_b @ u_axis
        edge_v_b = edge_b @ v_axis

    n_edges = len(edge_u_a)
    n_samples = samples.shape[0]

    # --- 4. Compute min distance from each sample to nearest boundary edge (vectorised) ---
    # For each sample, compute distance to every edge, take the min.
    # Vectorise over edges; loop over sample chunks to control memory.
    ...
    for start in range(0, n_samples, chunk_size):
        ...
        su = samples_u[start:end, None]  # (C, 1)
        sv = samples_v[start:end, None]  # (C, 1)

        # Edge vectors
        eu_a = edge_u_a[None, :]   # (1, E)
        ev_a = edge_v_a[None, :]
        eu_b = edge_u_b[None, :]
        ev_b = edge_v_b[None, :]
        ...
        du = eu_b - eu_a   # (1, E)
        dv = ev_b - ev_a
        len_sq = du * du + dv * dv   # (1, E)
        len_sq = np.maximum(len_sq, 1e-18)

        # Parameter t of closest point on segment
        t = ((su - eu_a) * du + (sv - ev_a) * dv) / len_sq  # (C, E)
        t = np.clip(t, 0.0, 1.0)

        # Closest point on segment
        proj_u = eu_a + t * du  # (C, E)
        proj_v = ev_a + t * dv

        dist_sq = (su - proj_u) ** 2 + (sv - proj_v) ** 2   # (C, E)
        chunk_min = np.sqrt(dist_sq.min(axis=1))             # (C,)

        min_dist[start:end] = chunk_min

    # --- 5. Threshold: keep inner keep_ratio ---
    max_bd = min_dist.max()
    if max_bd < 1e-8:
        # All points are on the boundary (degenerate)
        print("[DoorPush] All samples at zero boundary distance; keeping all.")
        return samples

    # The "margin" we remove is (1 - keep_ratio) of the max boundary distance.
    # e.g. keep_ratio=0.7 → remove samples with boundary distance < 0.3 * max_bd
    margin = (1.0 - keep_ratio) * max_bd
    keep_mask = min_dist >= margin

    inner = samples[keep_mask]
    print(f"[DoorPush] 2D boundary inset: max_bd={max_bd:.4f}, "
          f"margin={margin:.4f} ({(1-keep_ratio)*100:.0f}%), "
          f"kept {inner.shape[0]}/{n_samples} samples")

    return inner

def filter_samples_far_from_hinge(
    samples: np.ndarray,
    hinge_point: np.ndarray,
    hinge_axis: np.ndarray,
    keep_far_ratio: float = 0.5,
) -> np.ndarray:
    """
    For revolute joints: discard the closest `1 - keep_far_ratio` of samples
    to the hinge axis line, keeping only the far portion with better leverage.

    The distance is the perpendicular distance from each sample to the
    infinite line defined by (hinge_point, hinge_axis).

    Args:
        samples: (S, 3) surface samples
        hinge_point: (3,) a point on the hinge axis (joint pivot)
        hinge_axis: (3,) unit direction of the hinge axis
        keep_far_ratio: fraction of samples to keep (0.5 = keep far 50%)

    Returns:
        far_samples: (K, 3) filtered samples
    """
    if samples.shape[0] == 0:
        return samples

    axis = np.asarray(hinge_axis, dtype=np.float64)
    axis /= np.linalg.norm(axis) + 1e-12
    pivot = np.asarray(hinge_point, dtype=np.float64)

    # Vector from pivot to each sample
    v = samples - pivot[None, :]                       # (S, 3)

    # Parallel component along axis
    parallel = (v @ axis)[:, None] * axis[None, :]     # (S, 3)

    # Perpendicular component = radial distance to hinge line
    perp = v - parallel                                # (S, 3)
    distances = np.linalg.norm(perp, axis=1)           # (S,)

    # Keep samples whose distance is in the top keep_far_ratio
    # i.e. discard the closest (1 - keep_far_ratio) fraction
    threshold = np.percentile(distances, (1.0 - keep_far_ratio) * 100.0)

    keep_mask = distances >= threshold
    far_samples = samples[keep_mask]

    print(f"[DoorPush] Hinge distance filter: "
          f"dist range [{distances.min():.4f}, {distances.max():.4f}], "
          f"threshold={threshold:.4f} ({(1-keep_far_ratio)*100:.0f}th pctl), "
          f"kept {far_samples.shape[0]}/{samples.shape[0]} samples")

    return far_samples

def compute_door_push_targets(
    door_root_path: str,
    outward_normal: np.ndarray,
    normal_angle_deg: float = 10.0,
    keep_ratio: float = 0.7,
    ...
    hinge_point: Optional[np.ndarray] = None,
    hinge_axis: Optional[np.ndarray] = None,
    hinge_keep_far_ratio: float = 0.5,
    visualize: bool = False,
    vis_prim_path: str = "/World/PushTargets",
    debug_print: bool = True,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Extract the flat outer panel of a door link mesh for push-based grasping.
    
    Returns
    -------
    panel_samples  : (S, 3) dense surface points on the full panel
    inner_samples  : (K, 3) dense surface points after all filtering (final push targets)
    panel_normal   : (3,)   unit outward normal
    """
    stage = omni.usd.get_context().get_stage()
    out_n = np.asarray(outward_normal, dtype=np.float64)
    out_n /= np.linalg.norm(out_n) + 1e-12
    ...
    # --- 1. Collect & weld ---
    points_raw, faces_raw = collect_mesh_data_under(door_root_path)
    points, faces = weld_vertices(points_raw, faces_raw, eps=weld_eps)
    if debug_print:
        print(f"[DoorPush] Mesh: {len(points_raw)}->{len(points)} verts, "
              f"{len(faces_raw)}->{len(faces)} tris (weld eps={weld_eps})")
    
    # --- 2. Per-face geometry ---
    centers, normals, areas = compute_face_geometry(points, faces)

    # --- 3. Panel face selection (area-weighted depth histogram) ---
    panel_fi = select_panel_faces(
        centers, normals, areas, faces, out_n,
    ...
    if len(panel_fi) == 0:
        print("[DoorPush] ERROR: No panel faces found.")
        return np.empty((0, 3)), np.empty((0, 3)), out_n

    # --- 4. Largest connected face component ---
    face_adj = build_face_adjacency(faces, panel_fi)
    panel_fi = largest_face_component(panel_fi, face_adj, areas)
    if debug_print:
        print(f"[DoorPush] Largest face component: {len(panel_fi)} faces")

    # --- 5. Dense surface sampling on panel triangles ---
    panel_samples = sample_panel_surface_points(
        points, faces, panel_fi, n_points=n_surface_samples,
    )
    if debug_print:
        print(f"[DoorPush] Sampled {panel_samples.shape[0]} surface points on panel")
    
    # --- 6. Filter dense samples to inner region (2D boundary inset) ---
    inner_samples = filter_samples_to_inner_region(
        panel_samples, out_n, panel_fi, points, faces,
        keep_ratio=keep_ratio,
    )
    if debug_print:
        print(f"[DoorPush] Inner-core surface samples: {inner_samples.shape[0]}")

    # --- 6b. For revolute joints: discard the half closest to the hinge ---
    if joint_type == "revolute" and hinge_point is not None and hinge_axis is not None:
        inner_samples = filter_samples_far_from_hinge(
            inner_samples,
            hinge_point=hinge_point,
            hinge_axis=hinge_axis,
            keep_far_ratio=hinge_keep_far_ratio,
        )
        if debug_print:
            print(f"[DoorPush] After hinge filter: {inner_samples.shape[0]} samples")
    elif joint_type == "revolute":
        print("[DoorPush] WARNING: joint_type='revolute' but hinge_point/hinge_axis not provided. "
              "Skipping hinge distance filter.")
    
    # --- 7. Visualization (optional) ---
    if visualize and inner_samples.shape[0] > 0:
        # Clean up old visualization
        old = stage.GetPrimAtPath(vis_prim_path)
        if old.IsValid():
            stage.RemovePrim(vis_prim_path)
        
        # Create visualization scope
        UsdGeom.Xform.Define(stage, Sdf.Path(vis_prim_path))
        
        # Visualize only final push target points (red spheres)
        vis_points_from_positions(
            inner_samples,
            f"{vis_prim_path}/push_targets",
            color=(1.0, 0.1, 0.1),  # Bright red
            width=0.008,  # Larger for visibility
        )
        
        if debug_print:
            print(f"[DoorPush] Visualized {inner_samples.shape[0]} push target points at {vis_prim_path}")

    return panel_samples, inner_samples, out_n

def vis_points_from_positions(positions, prim_path, color=(0, 1, 0), width=0.015):
    stage = omni.usd.get_context().get_stage()
    old = stage.GetPrimAtPath(prim_path)
    if old.IsValid():
        stage.RemovePrim(prim_path)
    pts_prim = UsdGeom.Points.Define(stage, Sdf.Path(prim_path))
    gf_pts = [Gf.Vec3f(*[float(v) for v in p]) for p in positions]
    if not gf_pts:
        return
    pts_prim.CreatePointsAttr(gf_pts)
    pts_prim.CreateWidthsAttr([width] * len(gf_pts))
    pts_prim.CreateDisplayColorPrimvar().Set(
        [Gf.Vec3f(*[float(c) for c in color])] * len(gf_pts))
...
def generate_and_filter_grasps_for_joint(stage,
    ...
    print(f"\n[INFO] Generating push grasp poses for {joint_name} -> {child_link_name}")
    
    #Step 1: Get Joint Info
    ...
    #Step 2: Compute Door Outward Normal
    root_path = resolve_asset_root_under_ref(stage, object_ref_path)
    if joint_type =="revolute":
        door_normal = compute_door_normal_from_joint(
            stage,
            joint_prim,
            joint_params
        )
    ...
    if door_normal is None:
        print(f"[ERROR] Could not compute door normal")
        return []
    
    # Step 3: Extract panel surface samples using door push algorithm
    asset_root = resolve_asset_root_under_ref(stage, object_ref_path)
    visuals_path = f"{asset_root}/{child_link_name}/visuals"
    
    # Step 4: Extract panel surface samples using door push algorithm
    try:
        panel_samples, inner_samples, refined_normal = compute_door_push_targets(
            door_root_path=visuals_path,
            outward_normal=door_normal,
            normal_angle_deg=PANEL_NORMAL_ANGLE_DEG,
            keep_ratio=PANEL_INNER_KEEP_RATIO,
            n_surface_samples=num_samples,
            joint_type=joint_type,
            hinge_point=pivot if joint_type == "revolute" else None,
            hinge_axis=axis if joint_type == "revolute" else None,
            hinge_keep_far_ratio=HINGE_KEEP_FAR_RATIO,
            visualize=True,
            vis_prim_path=f"/World/PushTargets/{joint_name}",
            debug_print=True,
        )
        
        # Use refined normal from panel extraction (more accurate than heuristic)
        door_normal = refined_normal
    ...
    if inner_samples.shape[0] == 0:
        print(f"[ERROR] No valid panel samples after filtering")
        return []
    
    print(f"[INFO] Extracted {inner_samples.shape[0]} push target points")
    push_points = inner_samples
    
    # Step 5: Convert push points to grasp poses
    # For push grasps: gripper approach direction = door normal
    # No tangent constraint needed (no finger line axis)
    grasp_poses = []
    
    for point in push_points:
        try:
            # Create quaternion with gripper local Z aligned to door normal
            quaternion = normal_to_gripper_quaternion(door_normal)
            
            # Offset gripper base back from contact point by fingertip offset
            # This places the gripper base at the correct distance for contact
            position, _ = offset_pose_along_local_z(
                point, 
                quaternion, 
    ...
    print(f"[INFO] Generated {len(grasp_poses)} push grasp poses")
...
def overshoot_reject(
    ...
    target_displacement: float,  # Now NEGATIVE for closing
    joint_type: str,
) -> bool:
    ...
    # For CLOSING: target_displacement < 0, so close_dir = -1
    close_dir = 1.0 if target_displacement >= 0.0 else -1.0
    desired = initial_joint_pos + should_displacement
    
    # Overshoot = moving TOO FAR in closing direction
    overshoot = close_dir * (float(cur) - desired)
...
def compute_target_joint_displacement(joint_params: Dict) -> float:
    ...
    if joint_params["joint_type"] == "revolute":
        ...
        target_delta -= CLOSE_OVERSHOOT_DEG 
        ...
        target_delta = CLOSE_SIGN * initial_pos * 2
        target_delta -= CLOSE_OVERSHOOT_M
...
def generate_trajectories_for_all_grasps(
    ...
    for joint_name, joint_data in all_grasps.items():
        ...
        target_displacement = compute_target_joint_displacement(joint_params)
...
async def process_one_object(obj_usd: Path, obj_id: str, output_dir: Path, object_wrapper_path: str):
    ...
    #filter out the inner meshes like the trays which require preliminaries 
    ...
    for joint_name, joint_type, child_link_name, lower, upper, joint_axis in joints_info:
        ...
        initial_pos = compute_initial_joint_position(joint_type, lower, upper)
        print(f"The initial pose is: {initial_pos}")
    ...
    all_grasps = generate_grasps_for_all_joints(
        ...
        num_samples_per_joint= NUM_POINTS_PER_JOINT
    )
...
async def validate_batch_parallel(
    ...
    for env_idx in range(len(batch)):
        ...
        print(f"[BATCH {batch_index}][ENV {env_idx}] Resetting joint '{joint_name}' to initial position...")
        ...
        joint_params = get_joint_world_parameters(stage, joint_prim)
        if joint_params is None:
            print(f"[WARN] Env {env_idx}: Could not get parameters for {joint_name}")
            continue
        
        # Compute initial position using min(absolute, ratio) logic
        initial_pos = compute_initial_joint_position(
            joint_params["joint_type"],
            joint_params["lower_limit"],
            joint_params["upper_limit"]
        )
    ...
    for env_idx in range(len(batch)):
        ...
        if stepback_pos[2] < ground_z:
            ...
            print(f"[REJECT] Env {env_idx}: Stepback z={stepback_pos[2]:.4f} below ground z={ground_z:.4f}")
            continue
    ...
    APPROACH_DOOR_ANGLE_TOLERANCE_DEG = 2.0  # Reject if door moves > 2 degrees during approach
    APPROACH_DOOR_DISTANCE_TOLERANCE_M = 0.01  # Reject if door moves > 1cm during approach

    # Record door positions at START of approach
    approach_start_positions = []
    for env_idx in range(len(batch)):
        if env_failed[env_idx]:
            approach_start_positions.append(None)
            continue
        
        joint_prim = initial_states[env_idx]['joint_prim']
        start_pos = get_joint_current_position(stage, joint_prim)
        approach_start_positions.append(start_pos)
    ...
    for env_idx in range(len(batch)):
        ...
        joint_prim = initial_states[env_idx]['joint_prim']
        approach_start_pos = approach_start_positions[env_idx]
        
        if approach_start_pos is None:
            continue
        
        approach_end_pos = get_joint_current_position(stage, joint_prim)
        if approach_end_pos is None:
            env_failed[env_idx] = True
            print(f"[REJECT] Env {env_idx}: Could not read door position at end of approach")
            continue
        
        door_movement = abs(approach_end_pos - approach_start_pos)
        traj = batch[env_idx]
        joint_type = traj.get("joint_type", "")
        
        if joint_type == "revolute":
            door_movement_deg = np.degrees(door_movement)
            if door_movement_deg > APPROACH_DOOR_ANGLE_TOLERANCE_DEG:
                env_failed[env_idx] = True
                print(f"[REJECT] Env {env_idx}: Door moved {door_movement_deg:.2f}° during approach (tolerance: {APPROACH_DOOR_ANGLE_TOLERANCE_DEG}°)")
                continue
        else:  # prismatic
            if door_movement > APPROACH_DOOR_DISTANCE_TOLERANCE_M:
                env_failed[env_idx] = True
                print(f"[REJECT] Env {env_idx}: Door moved {door_movement:.4f}m during approach (tolerance: {APPROACH_DOOR_DISTANCE_TOLERANCE_M}m)")
                continue
    ...
    # Track which envs finished early due to sufficient closure
    env_finished_early = [False] * len(batch)
    env_actual_steps = [0] * len(batch)  # Track actual steps completed before early termination
    ...
    for traj_step in range(max_traj_length):
        for env_idx in range(len(batch)):
            ...
            # >>> NEW: If finished early, hold gripper at frozen pose
            if env_finished_early[env_idx]:
                if env_freeze_pos[env_idx] is not None and env_freeze_quat[env_idx] is not None:
                    # Keep gripper frozen at termination position
                    gripper_wrapper_path = grip_wrap(env_idx)
                    set_gripper_world_pose(stage, gripper_wrapper_path, env_freeze_pos[env_idx], env_freeze_quat[env_idx])
                    set_kinematic_target_pose(stage, target_paths[env_idx], env_freeze_pos[env_idx], env_freeze_quat[env_idx])
        ...
        # Check for sufficient closure at EVERY step
        for env_idx in range(len(batch)):
            ...
            if current_joint_pos is not None:
                ...
                FREEZE_THRESHOLD_DEG = 0.5  # Stop when within 1 degrees of target
                FREEZE_THRESHOLD_M = 0.005  # Stop when within 1cm of target
                
                should_freeze = False
                
                if joint_type == "revolute":
                    if current_joint_pos < np.radians(FREEZE_THRESHOLD_DEG):
                        should_freeze = True
                else:  
                    if current_joint_pos < FREEZE_THRESHOLD_M:
                        should_freeze = True
                
                # >>> NEW: Freeze gripper when close to target
                if should_freeze:  # Closing motion
                    if current_joint_pos < initial_states[env_idx]['initial_joint_pos']:  # Moving in correct direction
                        env_finished_early[env_idx] = True
                        env_actual_steps[env_idx] = min(traj_step + 2, local_len - 1)
                        
                        # Store current gripper pose to freeze it
        ...
        # Only check overshoot at evenly-spaced checkpoints (for envs still running)
    ...
    for env_idx in range(len(batch)):
        ...
        if env_failed[env_idx]:
            results.append((original_idx, False, None))  # >>> CHANGED: Added termination_step
            continue
        
        # If finished early, it's already validated as successful
        if env_finished_early[env_idx]:
            termination_step = env_actual_steps[env_idx]  # >>> NEW
            print(f"[BATCH {batch_index}] Env {env_idx}: SUCCESS (early termination at step {termination_step})")
            results.append((original_idx, True, termination_step))  # >>> CHANGED: Include termination step
            continue
        
        # Otherwise, check final position as before
        joint_prim = initial_states[env_idx]['joint_prim']
        ...
        if final_joint_pos is None:
            results.append((original_idx, False, None))  # >>> CHANGED
            continue
        ...
        if target_displacement < 0:  # Closing motion
            # Verify we actually moved in closing direction
            if final_joint_pos >= initial_joint_pos:
                is_valid = False 
        ...
        # >>> CHANGED: Return None for trajectories that completed fully (no early termination)
