...
SINGLE_OBJECT_USD = _path_from_env("XHAND_OPEN_EDGE_OBJECT_USD", _THIS_DIR / "7120" / "Object.usd")

INPUT_DATASET_PATH = _path_from_env("XHAND_OPEN_EDGE_DATASET_PATH", _THIS_DIR)
HAND_USD = _path_from_env("XHAND_OPEN_EDGE_HAND_USD", _WORKSPACE_ROOT / "assets" / "hands" / "xhand" / "xhand_r.usd")
...
RIGHT_HAND_CLOSE_WBC = {
    "right_hand_index_bend_joint": 0.123918,
    "right_hand_index_joint1": 1.213004,
    "right_hand_index_joint2": 0.614356,
    "right_hand_mid_joint1": 1.224621,
    "right_hand_mid_joint2": 0.638591,
    "right_hand_pinky_joint1": 1.175954,
    "right_hand_pinky_joint2": 0.968658,
    "right_hand_ring_joint1": 1.151917,
    "right_hand_ring_joint2": 0.872665,
    "right_hand_thumb_bend_joint": 1.652828,
    "right_hand_thumb_rota_joint1": 0.349066,
    "right_hand_thumb_rota_joint2": 0.902335,
}
...
def save_trajectories_to_json(
    ...
    json_path = annotation_dir / "xhand_open_on_edge_trajectory.json"
...
def compute_hand_base_position_from_eef_target(
    ...
    """
    Convert a desired EEF world position into the hand-root world position.

    Args:
        target_eef_position: [3] desired EEF position in world coordinates
        hand_quaternion_wxyz: [4] hand root orientation [w, x, y, z]
        eef_local_offset: [3] offset from hand root to EEF in hand local coordinates

    Returns:
        [3] hand root position in world coordinates
    """
    ...
    return target_eef_position - world_eef_offset
...
def collect_all_child_link_surface_samples(
    stage,
    object_ref_path: str,
    child_link_name: str,
    n_points_per_mesh: int = 500,
) -> np.ndarray:
    """Collect mesh points from the full child-link visual subtree in world coordinates."""
    asset_root = resolve_asset_root_under_ref(stage, object_ref_path)
    visuals_root_path = f"{asset_root}/{child_link_name}/visuals"
    link_root_path = f"{asset_root}/{child_link_name}"

    target_path = visuals_root_path
    if not stage.GetPrimAtPath(target_path).IsValid():
        target_path = link_root_path
        if not stage.GetPrimAtPath(target_path).IsValid():
            print(f"[WARNING] No visuals or link root found for {child_link_name}")
            return np.zeros((0, 3), dtype=np.float64)

    max_points = max(int(n_points_per_mesh), 1) * 8
    sampled_points = collect_mesh_points_world(stage, target_path, max_points=max_points)
    if sampled_points.shape[0] == 0:
        print(f"[WARNING] No mesh points sampled from {target_path}")
        return np.zeros((0, 3), dtype=np.float64)

    print(
        f"[INFO] Total sampled points from {child_link_name}: "
        f"{sampled_points.shape[0]} (root={target_path})"
    )
...
def compute_prismatic_door_normal(
    ...
    hinge_pos = get_joint_hinge_position(stage, object_ref_path, child_link_name)
    if hinge_pos is None:
        print("[WARNING] Could not get hinge position for prismatic normal")
...
def filter_revolute_edge_points(
    ...
    """Keep outer-rim edge points on revolute links and return normals + edge-line directions."""
    ...
    for point in surface_points:
        ...
        radial_distances.append(np.linalg.norm(perpendicular))
        perpendiculars.append(perpendicular)

    axis_positions = np.asarray(axis_positions, dtype=np.float64)
    radial_distances = np.asarray(radial_distances, dtype=np.float64)
    perpendiculars = np.asarray(perpendiculars, dtype=np.float64)
    ...
    max_abs_dot = np.sin(np.deg2rad(3.0))
    angle_ok_mask = []
    for point in surface_points:
        v = point - pivot
        ...
        if v_norm < 1e-8:
            ...
            v_dir = v / v_norm
            angle_ok_mask.append(abs(np.dot(v_dir, door_normal)) <= max_abs_dot)
    angle_ok_mask = np.asarray(angle_ok_mask, dtype=bool)
    ...
    for i in range(num_slices):
        ...
        local_max_radius = radial_distances[valid_in_slice].max()
        local_threshold = local_max_radius - epsilon
        keep_mask[valid_in_slice] |= radial_distances[valid_in_slice] >= local_threshold
    ...
    # For the selected rim band, the edge line itself runs along the hinge axis.
    edge_directions = np.tile(axis, (filtered_points.shape[0], 1))

    return (
        filtered_points,
        np.asarray(normals, dtype=np.float64),
        np.asarray(edge_directions, dtype=np.float64),
    )
...
def filter_prismatic_edge_points(
    ...
    """Keep top-edge points on prismatic links and return normals + edge directions."""
    if surface_points.shape[0] == 0:
        return (
            np.zeros((0, 3), dtype=np.float64),
            np.zeros((0, 3), dtype=np.float64),
    ...
    upward_normal = world_up - np.dot(world_up, axis) * axis
    if np.linalg.norm(upward_normal) < 1e-6:
        world_up = np.array([1.0, 0.0, 0.0], dtype=np.float64)
        upward_normal = world_up - np.dot(world_up, axis) * axis
    upward_normal = upward_normal / (np.linalg.norm(upward_normal) + 1e-12)

    max_abs_dot = np.sin(np.deg2rad(3.0))
    upward_projections = []
    angle_ok_mask = []
    for point in surface_points:
        upward_projections.append(np.dot(point, upward_normal))
        v = point - pivot
        v_norm = np.linalg.norm(v)
        if v_norm < 1e-8:
            angle_ok_mask.append(False)
    ...
    upward_projections = np.asarray(upward_projections, dtype=np.float64)
    angle_ok_mask = np.asarray(angle_ok_mask, dtype=bool)
    ...
    edge_directions = np.tile(axis, (filtered_points.shape[0], 1))
    print(f"[INFO] Prismatic edge filter: {filtered_points.shape[0]}/{surface_points.shape[0]} points kept")
    return filtered_points, normals, edge_directions

def normal_and_tangent_to_hand_quaternion(
    normal: np.ndarray,
    tangent: np.ndarray,
    door_normal: Optional[np.ndarray] = None,
) -> np.ndarray:
    """
    Build a Dex3.1 hand orientation from edge geometry.

    The hand is oriented such that:
    - HAND_PALM_DIRECTION aligns with -edge_normal
    - HAND_FINGER_DIRECTION is perpendicular to the edge line
    """
    normal = normal / (np.linalg.norm(normal) + 1e-12)
    tangent = tangent / (np.linalg.norm(tangent) + 1e-12)

    world_palm_dir = -normal
    world_finger_dir = np.cross(world_palm_dir, tangent)
    finger_norm = np.linalg.norm(world_finger_dir)
    if finger_norm < 1e-6:
        raise ValueError(
            f"[ERROR] Edge line is parallel to palm direction - cannot constrain hand orientation!\n"
            f"  normal: {normal}\n"
            f"  tangent: {tangent}"
        )
    world_finger_dir = world_finger_dir / finger_norm

    # The perpendicular finger direction has a sign ambiguity. Pick the branch
    # whose finger vector points opposite to the outward door normal.
    if door_normal is not None:
        door_normal = np.asarray(door_normal, dtype=np.float64)
        door_normal = door_normal / (np.linalg.norm(door_normal) + 1e-12)
        if np.dot(world_finger_dir, door_normal) > 0.0:
            world_finger_dir = -world_finger_dir
    ...
    local_frame = np.stack([palm_local, finger_local, third_local], axis=1)
    world_frame = np.stack([world_palm_dir, world_finger_dir, third_world], axis=1)
    rotation_matrix = world_frame @ local_frame.T
...
def sample_eef_poses_on_edge(
    ...
    """Convert sampled edge points into hand root poses by matching the hand EEF to each point."""
    edge_points = np.asarray(edge_points, dtype=np.float64)
    edge_normals = np.asarray(edge_normals, dtype=np.float64)
    edge_directions = np.asarray(edge_directions, dtype=np.float64)

    if edge_points.shape[0] == 0:
        return []

    if edge_points.shape[0] > num_samples:
        idx = np.random.choice(edge_points.shape[0], size=num_samples, replace=False)
        edge_points = edge_points[idx]
        edge_normals = edge_normals[idx]
        edge_directions = edge_directions[idx]

    poses = []
    skipped_count = 0
    for point, normal, tangent in zip(edge_points, edge_normals, edge_directions):
        try:
            quaternion = normal_and_tangent_to_hand_quaternion(normal, tangent, door_normal=door_normal)
            position = compute_hand_base_position_from_eef_target(point, quaternion, eef_local_offset)
    ...
    if skipped_count > 0:
        print(
            f"[WARNING] Skipped {skipped_count}/{edge_points.shape[0]} grasps "
            f"due to degenerate orientations"
        )
    print(f"[INFO] Generated {len(poses)} EEF-aligned edge grasp poses")
    return poses
...
def generate_and_filter_grasps_for_joint(
    ...
    """Generate Dex-hand grasp poses from edge points on the articulated child link."""
    ...
    if joint_type == "revolute":
        ...
        filtered_points, normals, tangents = filter_prismatic_edge_points(
            child_link_points,
            pivot,
            axis,
            stage,
    ...
    eef_local_offset = get_hand_eef_local_offset(
        stage,
        GRIPPER_REF_PATH,
    ...
    all_grasp_poses = sample_eef_poses_on_edge(
        filtered_points,
        normals,
        tangents,
    ...
    print(f"[INFO] Total raw edge poses: {len(all_grasp_poses)}")
...
async def process_one_object(obj_usd: Path, obj_id: str, output_dir: Path, object_wrapper_path: str):
    ...
    reach_reset_joints = find_revolute_prismatic_joints_in_urdf(urdf_path)
    if not reach_reset_joints:
        overrides = MANUAL_HANDLE_OVERRIDES.get(obj_id, [])
        if overrides:
            reach_reset_joints = _build_joints_info_from_overrides(urdf_path, overrides)

    for joint_name, joint_type, child_link_name, lower, upper, joint_axis, handle_visual_indices in reach_reset_joints:
        joint_prim = find_usd_joint_prim(stage, OBJECT_REF_PATH, joint_name)
        if joint_prim is None:
            continue
    ...
    print(f"[INFO] Joints set to nonzero initial positions, ready for grasp generation")
    ...
    # Keep the cloned template clean. Grasp generation uses a nonzero joint
    # state, but cloning while the live physics scene still holds that state can
    # corrupt multi-env PhysX initialization. Reset env_0 before GridCloner.
    print("[INFO] Resetting template object joints before cloning...")
    for joint_name, joint_type, child_link_name, lower, upper, joint_axis, handle_visual_indices in reach_reset_joints:
        joint_prim = find_usd_joint_prim(stage, OBJECT_REF_PATH, joint_name)
        if joint_prim is None:
            continue
        ...
        reset_pos = 0.0
        drive_kind = "angular" if joint_type == "revolute" else "linear"
        set_usd_joint_drive_target(stage, joint_prim.GetPath().pathString, reset_pos, drive_kind)
        set_joint_position_direct(stage, joint_prim, reset_pos)
...
def set_usd_joint_drive_target(stage, joint_prim_path: str, target_value: float, drive_kind: str = "linear"):
    ...
    try:
        ...
        # Always enforce safe gain caps — never trust baked-in USD values.
        # xhand_r.usd has high stiffness that causes GPU contact buffer overflow
        # (CUDA 700) when joints make unexpected contact during approach.
...
def set_hand_joint_drive_target(stage, gripper_ref_path: str, joint_name: str, target_radians: float):
    ...
    set_usd_joint_drive_target(
        stage,
        joint_path,
        float(np.degrees(target_radians)),
        drive_kind="angular",
    )
...
def compute_stepback_and_approach_poses(
    ...
    q_start = grasp_quaternion / (np.linalg.norm(grasp_quaternion) + 1e-12)
    q_end   = grasp_quaternion / (np.linalg.norm(grasp_quaternion) + 1e-12)

    THUMB_BEND_IDX = RIGHT_HAND_JOINT_NAMES.index("right_hand_thumb_bend_joint")
    THUMB_BEND_EARLY_FRACTION = 0.7  # thumb bend completes at 70% of approach
    ...
    for i in range(N):
        ...
        # thumb bend finishes early (at THUMB_BEND_EARLY_FRACTION of approach)
        alpha_thumb = min(alpha / THUMB_BEND_EARLY_FRACTION, 1.0)
...
async def validate_batch_parallel(
    ...
    if timeline.is_playing():
        timeline.stop()
        await step_simulation(30)  # wait for GPU to fully flush before mutating state

    # Park unused environments far away so their stale physics objects don't
    # keep simulating and corrupting the shared GPU solver context.
    PARK_POS = np.array([0.0, 0.0, -9999.0], dtype=np.float64)
    PARK_QUAT = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float64)
    for env_idx in range(len(batch), NUM_COPIES):
    ...
    for env_idx in range(len(batch)):
        ...
        # Reset joint to a reachable nonzero initial position
        joint_prim = find_usd_joint_prim(stage, object_ref_path, joint_name)
        if joint_prim:
            joint_params = get_joint_world_parameters(stage, joint_prim)
            if joint_params:
    ...
    for step_idx in range(MOVE_STEPS + 1):
        for env_idx in range(len(batch)):
            ...
            # --- Drive all right hand joints to interpolated open angles ---
