...
SINGLE_OBJECT_USD = _path_from_env("DEX3_OPEN_EDGE_OBJECT_USD", _THIS_DIR / "7120" / "Object.usd")

INPUT_DATASET_PATH = _path_from_env("DEX3_OPEN_EDGE_DATASET_PATH", _THIS_DIR)
HAND_USD = _path_from_env("DEX3_OPEN_EDGE_HAND_USD", _THIS_DIR / "dex3_1_right.usd")
...
def save_trajectories_to_json(
    ...
    json_path = annotation_dir / "dex3_1_open_on_edge_trajectory.json"
...
def compute_stepback_and_approach_poses(
    ...
    for i in range(N):
        ...
        # Reach 50% of the close pose over the first half of the approach,
        # then reach 70% by the end of the approach. The later close phase
        # completes the remaining 30% to the full close pose.
        if alpha <= 0.5:
            joint_alpha = alpha
        else:
            joint_alpha = 0.5 + (alpha - 0.5) * (0.2 / 0.5)
        joint_alpha = min(max(joint_alpha, 0.0), 0.7)
