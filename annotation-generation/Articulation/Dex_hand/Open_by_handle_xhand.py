...
from omni.isaac.dynamic_control import _dynamic_control as _dc_module
...
USE_GPU_PHYSX = _env_flag("XHAND_USE_GPU_PHYSX", default=False)
NUM_COPIES = int(os.environ.get("XHAND_NUM_COPIES", "2"))  # XHand is contact-heavy; override with XHAND_NUM_COPIES.
CLONE_SPACING = 3.0    # Spacing between clones in cloner grid (m)
TRAJ_OVERSHOOT_CHECKS   = 5 
NUM_SAMPLES_PER_JOINT = 10
TIMELINE_FLUSH_STEPS = 45
BATCH_DETACH_STEPS = 12
BATCH_RESET_SETTLE_STEPS = 60
RESOURCE_DIAG = _env_flag("XHAND_RESOURCE_DIAG", default=False)

# Hand joint drive settings
...
DISABLE_HAND_SELF_COLLISIONS = _env_flag("XHAND_DISABLE_HAND_SELF_COLLISIONS", default=True)
# Gripper approach axis used for filtering (local axis in gripper pose frame)
HAND_PALM_DIRECTION = (0, 1, 0)
HAND_FINGER_DIRECTION = (0, 0, -1)
HAND_FORWARD_LEAN_DEG = 15.0
THUMB_BEND_COMPLETE_FRACTION = 0.7
DELAYED_CLOSE_JOINT_NAMES = {
    "right_hand_pinky_joint2",
    "right_hand_mid_joint2",
    "right_hand_ring_joint2",
    "right_hand_index_joint2",
}
DELAYED_CLOSE_EARLY_TIME_FRACTION = 0.8
DELAYED_CLOSE_EARLY_DELTA_FRACTION = 0.6
THUMB_ROTA2_DELAYED_CLOSE_JOINT_NAME = "right_hand_thumb_rota_joint2"
THUMB_ROTA2_DELAYED_CLOSE_EARLY_TIME_FRACTION = 0.9
THUMB_ROTA2_DELAYED_CLOSE_EARLY_DELTA_FRACTION = 0.4
...
SINGLE_OBJECT_USD = _path_from_env("XHAND_OPEN_HANDLE_OBJECT_USD", _THIS_DIR / "7120" / "Object.usd")

INPUT_DATASET_PATH = _path_from_env("XHAND_OPEN_HANDLE_DATASET_PATH", _THIS_DIR)
HAND_USD = _path_from_env("XHAND_OPEN_HANDLE_HAND_USD", _WORKSPACE_ROOT / "assets" / "hands" / "xhand" / "xhand_r.usd")
...
RIGHT_HAND_CLOSE_WBC = {
    "right_hand_index_bend_joint": 0.066323, #3.8
    "right_hand_index_joint1": 1.213004, #69.5
    "right_hand_index_joint2": 1.719149, #98.5
    "right_hand_mid_joint1": 1.389282, #79.6
    "right_hand_mid_joint2": 1.206023, #69.1
    "right_hand_pinky_joint1": 1.511455, #86.6
    "right_hand_pinky_joint2": 1.059415, #60.7
    "right_hand_ring_joint1": 1.474803, #84.5
    "right_hand_ring_joint2": 1.108284, #63.5
    "right_hand_thumb_bend_joint": 1.832596, #105
    "right_hand_thumb_rota_joint1": 0.619592, #35.5
    "right_hand_thumb_rota_joint2": 1.129228, #64.7
}
...
def hand_pose_to_tensor(pose: dict, device="cpu") -> torch.Tensor:
    """Convert a hand pose dict to an ordered tensor."""
    return torch.tensor(
...
def _get_or_create_usd_attr(attr, create_fn):
    if not attr or not attr.IsValid():
        attr = create_fn()
    return attr


def resolve_hand_structure(stage, container_path: str) -> Optional[Dict[str, List[str]]]:
    """Resolve the authored XHand joints under a hand reference/container path."""
    ...
    for prim in Usd.PrimRange(root):
        if prim.GetName() not in RIGHT_HAND_JOINT_NAMES:
            continue
        if not (prim.IsA(UsdPhysics.RevoluteJoint) or prim.IsA(UsdPhysics.PrismaticJoint)):
            continue
        joint_paths_by_name[prim.GetName()] = prim.GetPath().pathString

    ordered_joint_paths = [
        joint_paths_by_name[name]
        for name in RIGHT_HAND_JOINT_NAMES
        if name in joint_paths_by_name
    ]
    if not ordered_joint_paths:
        return None

    missing = [name for name in RIGHT_HAND_JOINT_NAMES if name not in joint_paths_by_name]
    if missing:
        print(f"[WARN] Missing hand joints under {container_path}: {missing}")

    return {"ordered_joint_paths": ordered_joint_paths}


def configure_hand_joint_drives(stage, container_path: str) -> int:
    """Apply configured drive parameters to authored hand joints only."""
    ...
    for joint_path in structure["ordered_joint_paths"]:
        ...
        if joint_prim.IsA(UsdPhysics.RevoluteJoint):
            drive_kind = "angular"
            # XHand gains are authored as USD/PhysX drive gains already. Do not
            # scale them by rad/deg here; doing so turns 20/2 into ~0.35/~0.035
            # and leaves the fingers too weak to create holding friction.
        ...
        try:
            ...
            st = _get_or_create_usd_attr(drive.GetStiffnessAttr(), drive.CreateStiffnessAttr)
            st.Set(float(stiffness))

            dm = _get_or_create_usd_attr(drive.GetDampingAttr(), drive.CreateDampingAttr)
            dm.Set(float(damping))

            mf = _get_or_create_usd_attr(drive.GetMaxForceAttr(), drive.CreateMaxForceAttr)
            mf.Set(float(JOINT_MAX_FORCE))

            tp = _get_or_create_usd_attr(drive.GetTargetPositionAttr(), drive.CreateTargetPositionAttr)
            tp.Set(0.0)

            tv = _get_or_create_usd_attr(drive.GetTargetVelocityAttr(), drive.CreateTargetVelocityAttr)
            tv.Set(0.0)
            ...
            mv = _get_or_create_usd_attr(
                physx_joint.GetMaxJointVelocityAttr(),
                physx_joint.CreateMaxJointVelocityAttr,
            )
            mv.Set(float(max_velocity))

            arm = _get_or_create_usd_attr(physx_joint.GetArmatureAttr(), physx_joint.CreateArmatureAttr)
...
def configure_hand_articulation_physx(stage, container_path: str) -> int:
    """Apply PhysX articulation settings to the hand root(s)."""
    if not DISABLE_HAND_SELF_COLLISIONS:
        return 0

    root = stage.GetPrimAtPath(container_path)
    if not root.IsValid():
        print(f"[WARN] Could not resolve hand root for PhysX articulation setup: {container_path}")
        return 0
    ...
    for prim in Usd.PrimRange(root):
        ...
        physx_art = PhysxSchema.PhysxArticulationAPI(prim)
        physx_art.CreateEnabledSelfCollisionsAttr().Set(False)
        configured += 1

    if configured:
        print(f"[INFO] Disabled self-collisions on {configured} hand articulation root(s) under {container_path}")
    else:
        print(f"[WARN] No hand articulation root found under {container_path}")
    return configured
...
async def stop_timeline_and_flush(reason: str, steps: int = TIMELINE_FLUSH_STEPS):
    """Stop PhysX cleanly and give Kit a few updates to release simulation-side resources."""
    if timeline.is_playing():
        print(f"[DEBUG] Stopping timeline ({reason})...")
        timeline.stop()
    for _ in range(max(0, int(steps))):
        await omni.kit.app.get_app().next_update_async()

async def ensure_timeline_playing():
    """Start the timeline only when the simulation is not already running."""
    if not timeline.is_playing():
        print(f"[DEBUG] Timeline stopped, starting physics...")
        timeline.play()
        await omni.kit.app.get_app().next_update_async()
...
def print_resource_diag(stage, label: str):
    if not RESOURCE_DIAG:
        return
    ...
    rss = _process_rss_mb()
    rss_msg = f", rss={rss:.1f}MB" if rss is not None else ""
    print(f"[DIAG] {label}: {counts}{rss_msg}")
...
def setup_physics_scene(stage):
    ...
    # XHand creates many simultaneous contacts. Default Isaac Sim PhysX uses the
    # GPU pipeline, but repeated start/stop cycles under heavy contact can leave
    # the CUDA context poisoned after an illegal access. Keep the stable CPU path
    # by default; opt back into GPU with XHAND_USE_GPU_PHYSX=1.
    ...
    if USE_GPU_PHYSX:
        ...
        physx_scene_api.CreateGpuTempBufferCapacityAttr().Set(256 * 1024 * 1024)
        physx_scene_api.CreateGpuMaxRigidContactCountAttr().Set(4 * 1024 * 1024)
        physx_scene_api.CreateGpuMaxRigidPatchCountAttr().Set(1 * 1024 * 1024)
        physx_scene_api.CreateGpuHeapCapacityAttr().Set(512 * 1024 * 1024)
        physx_scene_api.CreateGpuFoundLostPairsCapacityAttr().Set(1 * 1024 * 1024)
        physx_scene_api.CreateGpuFoundLostAggregatePairsCapacityAttr().Set(1 * 1024 * 1024)
        physx_scene_api.CreateGpuTotalAggregatePairsCapacityAttr().Set(1 * 1024 * 1024)
        physx_scene_api.CreateGpuMaxNumPartitionsAttr().Set(8)
        physx_scene_api.CreateGpuCollisionStackSizeAttr().Set(256 * 1024 * 1024)
        print("[INFO] PhysX scene: GPU dynamics enabled with enlarged XHand buffers")
    else:
        physx_scene_api.CreateEnableGPUDynamicsAttr().Set(False)
        physx_scene_api.CreateBroadphaseTypeAttr().Set("MBP")
        print("[INFO] PhysX scene: CPU dynamics/MBP broadphase (set XHAND_USE_GPU_PHYSX=1 to use GPU)")
...
def save_trajectories_to_json(
    ...
    Each waypoint: [x, y, z, w, qx, qy, qz, hand joints... (radians)]
    ...
    data = {
        ...
        "waypoint_format": f"x, y, z, w, qx, qy, qz, j0..j{len(RIGHT_HAND_JOINT_NAMES) - 1} (radians)",
        "trajectories": {}
    }
    ...
    json_path = annotation_dir / "xhand_open_by_handle_trajectory.json"
...
def collect_mesh_triangles_world(stage, root_path: str) -> Tuple[np.ndarray, np.ndarray]:
    """Collect triangulated mesh faces under root_path in world coordinates."""
    root_prim = stage.GetPrimAtPath(root_path)
    if not root_prim.IsValid():
    ...
    for prim in Usd.PrimRange(root_prim, Usd.TraverseInstanceProxies()):
        ...
        xf = _world_xf_of_prim(prim)
        pts_world = np.asarray([_gvec_to_np(xf.Transform(p)) for p in points], dtype=np.float64)

        cursor = 0
        for count in face_counts:
            count = int(count)
            face = [int(i) for i in face_indices[cursor:cursor + count]]
            cursor += count
            if count < 3:
                continue
            if any(i < 0 or i >= pts_world.shape[0] for i in face):
                continue

            p0 = pts_world[face[0]]
            for j in range(1, count - 1):
                tri = np.stack([p0, pts_world[face[j]], pts_world[face[j + 1]]], axis=0)
    ...
    return np.stack(triangles, axis=0), np.asarray(areas, dtype=np.float64)


def sample_mesh_surface_points_world(stage, root_paths: List[str], num_samples: int) -> np.ndarray:
    """Sample points uniformly by triangle surface area across one or more mesh roots."""
    all_triangles = []
    all_areas = []
    for root_path in root_paths:
        triangles, areas = collect_mesh_triangles_world(stage, root_path)
    ...
    u = np.random.random(size=int(num_samples))
    v = np.random.random(size=int(num_samples))
    sqrt_u = np.sqrt(u)
    b0 = 1.0 - sqrt_u
    b1 = sqrt_u * (1.0 - v)
    b2 = sqrt_u * v
...
def compute_prismatic_door_normal(
    stage,
    object_wrapper_path: str,
    object_ref_path: str,
    child_link_name: str,
    sliding_axis: np.ndarray,
) -> Optional[np.ndarray]:
    """For prismatic joints, choose the outward-facing direction parallel to the slide axis."""
    axis_norm = np.asarray(sliding_axis, dtype=np.float64)
    axis_len = np.linalg.norm(axis_norm)
    if axis_len < 1e-8:
        print(f"[PrismaticNormal] Degenerate slide axis for {child_link_name}")
        return None

    axis_norm = axis_norm / axis_len

    asset_root = resolve_asset_root_under_ref(stage, object_ref_path)
    link_path = f"{asset_root}/{child_link_name}"
    ...
    if door_center is not None:
        ...
        if np.linalg.norm(to_door) > 1e-8 and np.dot(axis_norm, to_door) < 0.0:
            axis_norm = -axis_norm

    print(f"[PrismaticNormal] slide_axis/outward_normal={axis_norm}")
    return axis_norm
...
def compute_handle_axis_pca(
    ...
    """
    pts = sample_mesh_surface_points_world(stage, handle_paths, max_points)
    if pts.shape[0] == 0:
...
def sample_eef_poses_on_handle(
    ...
    """
    # ------------------------------------------------------------------
    # 1. Sample points uniformly over handle mesh surface area
    # ------------------------------------------------------------------
    all_pts = sample_mesh_surface_points_world(stage, handle_paths, num_samples)
    if all_pts.shape[0] == 0:
        print(f"[WARN] sample_eef_poses_on_handle: no mesh points found")
        return []

    print(f"[INFO] Uniformly sampled {all_pts.shape[0]} handle surface points for EEF pose generation")
...
def create_and_bind_high_friction_material(
    ...
    search_prim_path: Optional[str] = None,
):
    """
    Create a physics material and bind it near instanceable mesh containers.

    Do not author bindings inside referenced instanceable asset internals. For
    every instanceable prim whose subtree contains meshes, bind the material on
    its immediate parent instead. That keeps the binding outside the instanceable
    prim while still close enough for descendants to inherit the physics material.
    
    Args:
        stage: USD stage
        root_prim_path: Non-instanceable wrapper/root prim path used as fallback
        material_name: Name for the material prim under /World/Physics_Materials
        ...
        search_prim_path: Asset/reference subtree to scan for instanceable mesh holders.
                          Defaults to root_prim_path.
    
    Returns:
        Tuple[int, List[str]]: Number of prims the material was bound to and an
        empty list kept for compatibility with the old restore flow.
    """
    safe_material_name = "".join(c if c.isalnum() or c == "_" else "_" for c in material_name)
    material_path = f"/World/Physics_Materials/{safe_material_name}"
    ...
    search_path = search_prim_path or root_prim_path
    search_prim = stage.GetPrimAtPath(search_path)
    if not search_prim.IsValid():
        print(f"[WARN] Invalid material search prim path {search_path}; falling back to {root_prim_path}")
        search_prim = root_prim
        search_path = root_prim_path
    ...
    def _is_instance_proxy(prim) -> bool:
        try:
            return bool(prim.IsInstanceProxy())
        except Exception:
            return False

    def _add_target(targets: Dict[str, str], prim, reason: str) -> bool:
        if not prim or not prim.IsValid():
            return False
        if prim.IsInstanceable() or _is_instance_proxy(prim):
            return False
        targets[prim.GetPath().pathString] = reason
        return True

    binding_targets: Dict[str, str] = {}
    instanceable_mesh_holders: Dict[str, str] = {}
    skipped_instanceable_parents: List[str] = []
    mesh_count = 0

    # Fallback bindings cover assets with no instanceable mesh container and
    # preserve inheritance from the non-instanceable wrapper/ref roots.
    _add_target(binding_targets, root_prim, "root fallback")
    if search_prim.GetPath() != root_prim.GetPath():
        _add_target(binding_targets, search_prim, "search root fallback")

    for prim in Usd.PrimRange(search_prim, Usd.TraverseInstanceProxies()):
        if not prim.IsValid() or not prim.IsA(UsdGeom.Mesh):
            continue
        mesh_count += 1

        ancestor = prim.GetParent()
        while ancestor and ancestor.IsValid() and ancestor.GetPath() != search_prim.GetPath():
            if ancestor.IsInstanceable():
                holder_path = ancestor.GetPath().pathString
                parent = ancestor.GetParent()
                if parent and parent.IsValid():
                    instanceable_mesh_holders[holder_path] = parent.GetPath().pathString
                    if not _add_target(binding_targets, parent, "parent above instanceable mesh holder"):
                        skipped_instanceable_parents.append(parent.GetPath().pathString)
            ancestor = ancestor.GetParent()

    bound_paths: List[str] = []
    failed_paths: List[str] = []
    for target_path in binding_targets.keys():
        target_prim = stage.GetPrimAtPath(target_path)
        if not target_prim.IsValid():
            failed_paths.append(target_path)
            continue
        try:
            bind_api = UsdShade.MaterialBindingAPI.Apply(target_prim)
            bind_api.Bind(material, materialPurpose="physics")
            bound_paths.append(target_path)
        except Exception as e:
            failed_paths.append(target_path)
            print(f"[WARN] Failed to bind material {material_path} to {target_path}: {e}")

    if not bound_paths:
        print(f"[WARN] No editable prims found to bind physics material {material_path}")
        return 0, []

    print(f"[INFO] Physics material '{material_path}' bound:")
    print(f"  - Search root: {search_path}")
    print(f"  - Meshes seen: {mesh_count}")
    print(f"  - Instanceable mesh holders: {len(instanceable_mesh_holders)}")
    if instanceable_mesh_holders:
        holder_items = list(instanceable_mesh_holders.items())
        print(f"  - Holder->parent sample: {holder_items[:min(4, len(holder_items))]}")
    print(f"  - Bound prims: {len(bound_paths)}")
    if bound_paths:
        print(f"  - Bound sample: {bound_paths[:min(6, len(bound_paths))]}")
    if skipped_instanceable_parents:
        unique_skipped = sorted(set(skipped_instanceable_parents))
        print(
            f"  - Skipped non-editable parent(s): "
            f"{unique_skipped[:min(6, len(unique_skipped))]}"
        )
    if failed_paths:
        print(f"  - Failed bindings: {failed_paths[:min(6, len(failed_paths))]}")
...
async def process_one_object(obj_usd: Path, obj_id: str, output_dir: Path, object_wrapper_path: str):
    ...
    if ctx.get_stage():
        ...
        await stop_timeline_and_flush("before closing previous stage")
        ...
        await step_simulation(TIMELINE_FLUSH_STEPS)
        
    print(f"[INFO] Creating new stage")
    await ctx.new_stage_async()
    _HAND_JOINT_PATH_CACHE.clear()  # stale paths from previous object
    stage = ctx.get_stage()
    ...
    # configure_hand_articulation_physx(stage, GRIPPER_REF_PATH)
    # await omni.kit.app.get_app().next_update_async()
    ...
    # The old overlap-filter implementation below is disabled. Do not prepare
    # temporary gripper mesh colliders while it is disabled: XHand has 180 visual
    # meshes, and authoring convex-decomposition APIs onto all of them creates a
    # large amount of unused collision/cooking state before cloning.
    gripper_instanceable_paths: List[str] = []
    print("[INFO] Skipping disabled overlap-filter collider prep for XHand")
    ...
    restore_instanceable(
        stage,
        object_material_instanceable_paths + changed_instanceable_paths,
    )
    await omni.kit.app.get_app().next_update_async()
    ...
    print_resource_diag(stage, "after cloning")

    print(f"[INFO] Configuring hand joint drives for all {NUM_COPIES} environments...")
    for i in range(NUM_COPIES):
        configure_hand_joint_drives(stage, grip_ref(i))
        configure_hand_articulation_physx(stage, grip_ref(i))
    await omni.kit.app.get_app().next_update_async()
    ...
    if all_trajectories:
        ...
        had_valid_trajectories = bool(valid_trajectories)
        ...
        if valid_trajectories and bottom_center_list:
            try:
                ...
                import traceback
                traceback.print_exc()
        # Release large numpy arrays from approach data
        apporach_data_map.clear()
        all_trajectories.clear()
        valid_trajectories.clear()
    else:
...
async def run_pipeline():
    ...
    print(f"# PhysX: {'GPU' if USE_GPU_PHYSX else 'CPU'} | NUM_COPIES={NUM_COPIES}")
    print(f"{'#'*80}\n")
...
_HAND_JOINT_PATH_CACHE = {}


def find_hand_joint_prim_path(stage, gripper_ref_path: str, joint_name: str) -> Optional[str]:
    """Find a hand joint by prim name under the referenced hand root."""
    cache_key = (gripper_ref_path, joint_name)
    cached_path = _HAND_JOINT_PATH_CACHE.get(cache_key)
    if cached_path:
        cached_prim = stage.GetPrimAtPath(cached_path)
        if cached_prim.IsValid():
            return cached_path

    direct_path = f"{gripper_ref_path}/joints/{joint_name}"
    direct_prim = stage.GetPrimAtPath(direct_path)
    if direct_prim.IsValid():
        _HAND_JOINT_PATH_CACHE[cache_key] = direct_path
        return direct_path

    root = stage.GetPrimAtPath(gripper_ref_path)
    if not root.IsValid():
        print(f"[WARN] find_hand_joint_prim_path: invalid hand root {gripper_ref_path}")
        return None
    ...
    for prim in Usd.PrimRange(root):
        ...
        _HAND_JOINT_PATH_CACHE[cache_key] = joint_path
        return joint_path

    print(f"[WARN] Could not find hand joint '{joint_name}' under {gripper_ref_path}")
    return None


def set_hand_joint_drive_target(stage, gripper_ref_path: str, joint_name: str, target_radians: float):
    joint_path = find_hand_joint_prim_path(stage, gripper_ref_path, joint_name)
    ...
    if joint_prim.IsA(UsdPhysics.RevoluteJoint):
        drive_kind = "angular"
        target_value = float(np.degrees(target_radians))
    elif joint_prim.IsA(UsdPhysics.PrismaticJoint):
        drive_kind = "linear"
        target_value = float(target_radians)
    else:
        return
    ...
    try:
        ...
        print(f"[WARN] Failed to set hand drive target on {joint_path}: {e}")


def set_hand_joint_runtime_state(stage, gripper_ref_path: str, joint_name: str, target_radians: float) -> bool:
    joint_path = find_hand_joint_prim_path(stage, gripper_ref_path, joint_name)
    if joint_path is None:
        return False
    ...
    return set_joint_position_runtime(stage, joint_prim, float(target_radians))

def set_joint_position_direct(stage, joint_prim: Usd.Prim, position: float):
    """
    Set joint initial position via JointStateAPI (USD layer).
    Called only when the timeline is stopped, so USD writes are safe and
    PhysX reads the values back when the simulation restarts.
    """
    if not joint_prim.IsValid():
    ...
    try:
        ...
        print(f"[ERROR] set_joint_position_direct failed on {joint_prim.GetPath()}: {e}")
        import traceback
        traceback.print_exc()


def _find_dynamic_control_dof(stage, joint_prim: Usd.Prim):
    if joint_prim is None or not joint_prim.IsValid():
        return None, _dc_module.INVALID_HANDLE, _dc_module.INVALID_HANDLE

    dc = _dc_module.acquire_dynamic_control_interface()
    articulation = dc.get_articulation(joint_prim.GetPath().pathString)
    if articulation == _dc_module.INVALID_HANDLE:
    ...
    if articulation == _dc_module.INVALID_HANDLE:
        return dc, articulation, _dc_module.INVALID_HANDLE

    dof_ptr = dc.find_articulation_dof(articulation, joint_prim.GetName())
    return dc, articulation, dof_ptr


def set_joint_position_runtime(stage, joint_prim: Usd.Prim, position: float, set_target: bool = True) -> bool:
    """Reset a live PhysX DOF without stopping/restarting the timeline."""
    try:
        dc, articulation, dof_ptr = _find_dynamic_control_dof(stage, joint_prim)
        if dc is None or articulation == _dc_module.INVALID_HANDLE or dof_ptr == _dc_module.INVALID_HANDLE:
            print(f"[WARN] Could not get live DOF handle for {joint_prim.GetPath()}")
            return False

        pos = float(position)
        dc.wake_up_articulation(articulation)
        dc.set_dof_state(dof_ptr, _dc_module.DofState(pos, 0.0, 0.0), _dc_module.STATE_ALL)
        if set_target:
            dc.set_dof_position_target(dof_ptr, pos)
        return True
    except Exception as e:
        print(f"[WARN] set_joint_position_runtime failed on {joint_prim.GetPath()}: {e}")
        return False
...
def compute_stepback_and_approach_poses(
    ...
    move_steps: int,
    hand_open_pose: np.ndarray = None,   # joint angles at step-back
    hand_close_pose: np.ndarray = None,  # joint angles at grasp
    ...
    Returns:
        ...
        joint_angles:[N, num_hand_joints]  linearly interpolated right hand joint angles
    """
    ...
    # At the step-back pose the hand is open; at the grasp pose it reaches
    # the requested pre-grasp/close configuration. Most joints interpolate
    # linearly for the whole approach, while the thumb bend closes early.
    # The delayed-close joints only complete 60% of their motion in the first
    # 80% of approach, then complete the final 40% in the last 20%.
    # The second thumb rotation completes the final 60% in the last 10%.
    if hand_open_pose is None:
        hand_open_pose  = RIGHT_HAND_OPEN_T.numpy()
    if hand_close_pose is None:
        hand_close_pose = RIGHT_HAND_OPEN_T.numpy()
    num_hand_joints = len(RIGHT_HAND_JOINT_NAMES)
    hand_open_pose = np.asarray(hand_open_pose, dtype=np.float64).reshape(num_hand_joints)
    hand_close_pose = np.asarray(hand_close_pose, dtype=np.float64).reshape(num_hand_joints)

    N = move_steps + 1
    positions    = np.zeros((N, 3),  dtype=np.float64)
    quaternions  = np.zeros((N, 4),  dtype=np.float64)
    joint_angles = np.zeros((N, num_hand_joints),  dtype=np.float64)
    ...
    thumb_bend_idx = RIGHT_HAND_JOINT_NAMES.index("right_hand_thumb_bend_joint")
    thumb_complete_step = max(1, int(np.floor(move_steps * THUMB_BEND_COMPLETE_FRACTION)))
    delayed_close_joint_indices = [
        idx
        for idx, joint_name in enumerate(RIGHT_HAND_JOINT_NAMES)
        if joint_name in DELAYED_CLOSE_JOINT_NAMES
    ]
    thumb_rota2_idx = RIGHT_HAND_JOINT_NAMES.index(THUMB_ROTA2_DELAYED_CLOSE_JOINT_NAME)
    ...
    for i in range(N):
        ...
        # --- Joint angles: linear interpolation during approach ---
        joint_angles[i] = hand_open_pose * (1.0 - alpha) + hand_close_pose * alpha

        if alpha <= DELAYED_CLOSE_EARLY_TIME_FRACTION:
            delayed_alpha = (
                DELAYED_CLOSE_EARLY_DELTA_FRACTION
                * alpha
                / DELAYED_CLOSE_EARLY_TIME_FRACTION
            )
        else:
            late_time_fraction = (alpha - DELAYED_CLOSE_EARLY_TIME_FRACTION) / (
                1.0 - DELAYED_CLOSE_EARLY_TIME_FRACTION
            )
            delayed_alpha = DELAYED_CLOSE_EARLY_DELTA_FRACTION + (
                1.0 - DELAYED_CLOSE_EARLY_DELTA_FRACTION
            ) * late_time_fraction
        delayed_alpha = float(np.clip(delayed_alpha, 0.0, 1.0))
        joint_angles[i, delayed_close_joint_indices] = (
            hand_open_pose[delayed_close_joint_indices] * (1.0 - delayed_alpha)
            + hand_close_pose[delayed_close_joint_indices] * delayed_alpha
        )

        if alpha <= THUMB_ROTA2_DELAYED_CLOSE_EARLY_TIME_FRACTION:
            thumb_rota2_alpha = (
                THUMB_ROTA2_DELAYED_CLOSE_EARLY_DELTA_FRACTION
                * alpha
                / THUMB_ROTA2_DELAYED_CLOSE_EARLY_TIME_FRACTION
            )
        else:
            late_time_fraction = (alpha - THUMB_ROTA2_DELAYED_CLOSE_EARLY_TIME_FRACTION) / (
                1.0 - THUMB_ROTA2_DELAYED_CLOSE_EARLY_TIME_FRACTION
            )
            thumb_rota2_alpha = THUMB_ROTA2_DELAYED_CLOSE_EARLY_DELTA_FRACTION + (
                1.0 - THUMB_ROTA2_DELAYED_CLOSE_EARLY_DELTA_FRACTION
            ) * late_time_fraction
        thumb_rota2_alpha = float(np.clip(thumb_rota2_alpha, 0.0, 1.0))
        joint_angles[i, thumb_rota2_idx] = (
            hand_open_pose[thumb_rota2_idx] * (1.0 - thumb_rota2_alpha)
            + hand_close_pose[thumb_rota2_idx] * thumb_rota2_alpha
        )

        # Thumb bend reaches the close target within the first 70% of approach
        # steps and then stays there for the rest of the approach.
        alpha_thumb = min(i / float(thumb_complete_step), 1.0)
        joint_angles[i, thumb_bend_idx] = (
            hand_open_pose[thumb_bend_idx] * (1.0 - alpha_thumb)
            + hand_close_pose[thumb_bend_idx] * alpha_thumb
        )
...
async def validate_batch_parallel(
    ...
    live_reset = timeline.is_playing()

    # Park all hands before resetting object joints. This breaks stale contacts
    # from the previous batch without tearing down and recreating the PhysX scene.
    PARK_POS = np.array([0.0, 0.0, 50.0], dtype=np.float64)
    PARK_QUAT = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float64)
    for env_idx in range(NUM_COPIES):
        set_gripper_world_pose(stage, grip_wrap(env_idx), PARK_POS, PARK_QUAT)

    print_resource_diag(stage, f"batch {batch_index} begin")

    if live_reset:
        for env_idx in range(NUM_COPIES):
            gripper_ref_path = grip_ref(env_idx)
            for hand_joint_name, target_angle in RIGHT_HAND_OPEN.items():
                set_hand_joint_drive_target(stage, gripper_ref_path, hand_joint_name, target_angle)
                set_hand_joint_runtime_state(stage, gripper_ref_path, hand_joint_name, target_angle)
        await step_simulation(BATCH_DETACH_STEPS)

    close_joints = hand_pose_to_tensor(RIGHT_HAND_POSES[HAND_CLOSE_CONFIG]).numpy()

    # Compute approach trajectories before reset so each hand can be placed at
    # its first safe step-back pose before the approach motion begins.
    all_approach_data = []
    ...
    for env_idx in range(len(batch)):
        ...
        if joint_prim:
            ...
            if joint_params:
                lower_limit = joint_params["lower_limit"]
                if live_reset:
                    if not set_joint_position_runtime(stage, joint_prim, lower_limit, set_target=False):
                        set_joint_position_direct(stage, joint_prim, lower_limit)
                else:
                    set_joint_position_direct(stage, joint_prim, lower_limit)

        # Start every validation from an open hand. Otherwise a previous batch can
        # leave XHand closed, and the next wrist reset may begin already in contact.
        gripper_ref_path = grip_ref(env_idx)
        for hand_joint_name, target_angle in RIGHT_HAND_OPEN.items():
            set_hand_joint_drive_target(stage, gripper_ref_path, hand_joint_name, target_angle)
            if live_reset:
        ...
        set_gripper_world_pose(
            stage,
            grip_wrap(env_idx),
            all_approach_data[env_idx]['positions'][0],
            all_approach_data[env_idx]['quats'][0],
        )
    
    await ensure_timeline_playing()
    await step_simulation(BATCH_RESET_SETTLE_STEPS)  # Let resets settle
    ...
    for step_idx in range(MOVE_STEPS + 1):
        for env_idx in range(len(batch)):
            ...
            # --- Drive all configured right hand joints to interpolated angles ---
            for joint_name, target_angle in zip(RIGHT_HAND_JOINT_NAMES, joint_targets):
                set_hand_joint_drive_target(stage, gripper_ref_path, joint_name, target_angle)

        await step_simulation(1)
            
    # Step 5: Close settle phase. The approach already ended at close_joints;
    # keep commanding that pose so the drives can settle without reopening.
    ...
    for traj_step in range(max_traj_length):
        ...
        for env_idx in range(len(batch)):
            ...
            if traj_step in traj_checkpoints and not env_failed[env_idx]:
                ...
                if overshoot_reject(
                    stage, joint_prim, initial_joint_pos,
                    should_disp, target_disp, joint_type,
                ):
    ...
    for env_idx in range(len(batch)):
        ...
        is_valid = (actual_displacement >= required_displacement) and (not out_of_bounds) and (not env_failed[env_idx])
    ...
    print_resource_diag(stage, f"batch {batch_index} end")
...
def get_joint_current_position(stage, joint_prim: Usd.Prim) -> Optional[float]:
    ...
    try:
        dc, articulation, dof_ptr = _find_dynamic_control_dof(stage, joint_prim)
        if dc is None or articulation == _dc_module.INVALID_HANDLE:
            print(f"[WARN] Could not get articulation handle for {joint_prim.GetPath()}")
            return None

        if dof_ptr != _dc_module.INVALID_HANDLE:
            dof_state = dc.get_dof_state(dof_ptr, _dc_module.STATE_POS)
            return float(dof_state.pos)
...
async def physics_validation_loop(
    ...
    try:
        ...
        await stop_timeline_and_flush("after physics validation")
