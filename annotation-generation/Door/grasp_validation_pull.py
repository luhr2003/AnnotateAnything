...
PRIMARY_ROTATE_ACCEPT_RATIO = 0.6
...
def get_hand_eef_local_offset(
    ...
    eef_prim = stage.GetPrimAtPath(f"{hand_ref_path}/{eef_link_name}")
    if not eef_prim.IsValid():
        print(f"[WARN] Hand EEF link not found at {hand_ref_path}/{eef_link_name}")
        return None
...
SINGLE_OBJECT_USD = _path_from_env("DOOR_PULL_OBJECT_USD", _THIS_DIR / "data" / "door_pull" / "Object.usd")

INPUT_DATASET_PATH = _path_from_env("DOOR_PULL_DATASET_PATH", _THIS_DIR / "data")
        ...
        for env_idx in range(len(batch)):
            ...
            for jn, ta in zip(RIGHT_HAND_JOINT_NAMES, joint_targets):
                set_usd_joint_drive_target(
                    stage, f"{grip_ref(env_idx)}/joints/{jn}",
